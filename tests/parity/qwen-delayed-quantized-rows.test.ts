...
describe.skipIf(!enabled)("Qwen delayed Quantized target row transaction", async () => {
  if (!enabled) return;
  ...
  type State = InstanceType<typeof SSMCache> | InstanceType<typeof DelayedQuantizedKVCache>;
  ...
  test("all target layers retain unequal prefixes and continue from the same per-row state", async () => {
    ...
    try {
      for (const tokens of [[1, 2, 3], [11, 12, 13, 14, 15, 16, 17]]) {
        ...
        try {
          ...
          base = solo.map((cache, layer) => {
            ...
            else { merged = new DelayedQuantizedKVCache(64, bits, 5, maintain); merged.mergeRows(previous ? [previous, cache] : [cache]); }
            previous?.dispose();
            return merged;
          });
      ...
      const clone = () => base.map(cache => {
        if (cache instanceof DelayedQuantizedKVCache) {
          const copy = new DelayedQuantizedKVCache(64, bits, 5, maintain); copy.mergeRows([cache]); return copy;
        }
      ...
      const compareRow = (actual: State[], reference: State[], row: number) => {
        for (let layer = 0; layer < actual.length; layer++) {
          ...
          } else if (a instanceof DelayedQuantizedKVCache && b instanceof DelayedQuantizedKVCache) {
            const x = a.extractRow(row), y = b.extractRow(row);
            try {
              expect(x.offset).toBe(y.offset);
              expect(x.minimumReusableOffset ?? 0).toBe(y.minimumReusableOffset ?? 0);
              const xx = x.state(), yy = y.state();
              try { expect(xx.map(digest)).toEqual(yy.map(digest)); }
              finally { if (x.stateNeedsDispose) for (const field of xx) field.dispose(); if (y.stateNeedsDispose) for (const field of yy) field.dispose(); }
      ...
      for (const accepted of [[0, 3], [3, 0], [1, 2], [3, 3]]) {
        ...
        try {
          ...
          for (let row = 0; row < 2; row++) {
            ...
            console.error(JSON.stringify({ accepted, row, offsets: actual.find(c => c instanceof DelayedQuantizedKVCache)?.rowOffsets }));
