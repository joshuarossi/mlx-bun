...
describe.skipIf(!enabled)("Qwen delayed TurboQuant target row transaction", async () => {
  if (!enabled) return;
  ...
  type State = InstanceType<typeof SSMCache> | InstanceType<typeof DelayedTurboQuantKVCache>;
  ...
  test("all target layers retain unequal prefixes and continue from the same per-row state", async () => {
    ...
    try {
      for (const tokens of [[1, 2, 3], [11, 12, 13, 14, 15, 16, 17]]) {
        ...
        try {
          ...
          base = solo.map((cache, layer) => {
            ...
            else { merged = new DelayedTurboQuantKVCache(8, 3, 5, maintain); merged.mergeRows(previous ? [previous, cache] : [cache]); }
            previous?.dispose();
            return merged;
          });
      ...
      const clone = () => base.map(cache => {
        if (cache instanceof DelayedTurboQuantKVCache) {
          const copy = new DelayedTurboQuantKVCache(8, 3, 5, maintain); copy.mergeRows([cache]); return copy;
        }
      ...
      const compareRow = (actual: State[], reference: State[], row: number) => {
        for (let layer = 0; layer < actual.length; layer++) {
          ...
          } else if (a instanceof DelayedTurboQuantKVCache && b instanceof DelayedTurboQuantKVCache) {
      ...
      for (const accepted of [[0, 3], [3, 0], [1, 2], [3, 3]]) {
        ...
        try {
          ...
          for (let row = 0; row < 2; row++) {
            ...
            console.error(JSON.stringify({ accepted, row, offsets: actual.find(c => c instanceof DelayedTurboQuantKVCache)?.rowOffsets }));
