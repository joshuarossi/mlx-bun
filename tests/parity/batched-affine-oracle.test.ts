...
for case in plan['cases']:
    ...
    for step in range(6):
        if step==3 and len(rows)>1:
            ...
            for c in cache:
                if isinstance(c.keys,(tuple,list)):
...
test.skipIf(!enabled)("full-attention affine rows match pinned same-B logits through reordering and retirement", async () => {
  ...
  const cases = [1,4].flatMap(width => [4,8,"mixed"].map(kind => ({ width,
