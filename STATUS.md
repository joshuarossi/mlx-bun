...
failure and SSD restart. TQ/MTP3 and KV4/MTP2 pass; automatic prefill sizing passes with all 19 responses/counts unchanged. Affine/TQ committed appends pass both Qwen artifacts and delayed conversion. Early first output is adopted after retained-state and monitored HTTP checks. Fixed-arrival two/four/eight-wave comparisons favor default throughput with exact outputs and documented stream-pause tradeoffs. M4 three-head KV4 attention grouping is integrated in 721ac4c after full-model and serving comparisons. The automatic-prefill Kanban task completes but misses the app-quality target. A four-arm replay isolates its initial divergence to chunk selection; early-output controls match. The full fixed-256 control is complete with unchanged source, identical first response/MTP counts, successful cache durability and 17/18 untouched-app acceptance. Keep fixed chunks in the task-comparison profile; the general automatic default is unchanged. This closes the requested integrated-task check. The full M4 standard AB/BA matrix is complete. A MiniCPM cached-completion gap is traced to repeated whole-history text conversion; exact-ID reuse improves both source-controlled pairs and passes the complete default-on standard matrix. Rotating mixed-KV same-B oracle coverage now passes on both machines; combined expansion/TQ/MTP pressure acceptance also passes on M4. Fused TurboQuant decoding is selected by default after both-machine shared-serving comparisons; quantization itself stays opt-in. R6 cache-enabled original/interleaved comparisons now pass all six pairs with exact responses/usage and durable SSD restart. Shared strict fill now passes native, delayed-precision, row-retirement and 108-response HTTP/cache acceptance. It improves the sequential fixture total by 13.50%/13.57% and stays within 0.13% of serial; concurrent latency tradeoffs remain. The current-version submission-threshold screen is complete; retain native defaults after mixed cross-model results. Gemma shared media preparation/decode passes both-machine native/HTTP checks, all 2,353 model-free tests and typechecks; downloads stay outside the native queue. M4 ABBA preserves all 78 paired responses and token counts, with faster concurrent completion and recorded lone-request/first-request latency costs. Qwen request-owned positions pass both-machine source/group/real-image checks. M4 image/video ABBA completes with exact full usage, faster concurrent completion, geometry-dependent text differences and recorded latency costs. Exact Qwen encoder objects now share RAM/SSD residency. Both-machine ownership/restart/HTTP and M4 source-controlled comparisons pass, with all responses/usage unchanged and a recorded first-use hash cost. Gemma encoder reuse passes both-machine identity/HTTP and M4 ABBA/fresh-process SSD acceptance; all 168 paired responses/usage match. R19 now has both-machine kernel costs and a constrained MLP allocation screen; retain k300, with real candidate quality and non-MLP work open. Gemma and Qwen media KV reuse pass both-machine native/RAM/SSD HTTP, fresh-process restore and M4 ABBA comparisons. Prepared media prefixes are selected by default, with exact Qwen responses and a recorded Gemma punctuation difference. R24 native activation quantization loses its both-machine operation screen; per-candidate memory/kernel requirements are recorded. Simple actual-weight low-rank/residual variants are rejected; packed differentiation needs a custom VJP. Calibrated quality/training and the remaining feature combinations stay open. Shared echo sampling/state/SSD checks pass on M1 Qwen/Gemma and M4 packed Qwen; Qwen/Gemma HTTP checks pass. The repeated M4 strict/echo ABBA preserves all 38 measured response/usage pairs and improves sentence/URL copies about 12%/41%; fill stays opt-in. A tool-turn primer probe fix restores small-Qwen prefix reuse. Mixed echo cohorts and independent greedy sampling pass both-machine correctness and M4 ABBA: all response/usage pairs match, sequential copy performance is flat and concurrent last completion improves with a per-request latency cost. Qwen echo+MTP is implemented through provider consumption of external tokens. Both-machine companion oracle, M4 HTTP/cache and six native configurations pass; M4 ABBA preserves all 38 measured responses/counts; URL copies improve 12–16%, short sentences cost 11–13%. The requested fresh MTP2+echo Kanban task completes with unchanged source and 17/18 untouched-app acceptance. Streaming rate is flat against the previous task; the model emits more tokens and executes seven failing checks in its own test script. All follow-ups hit the cache. Echo stays opt-in. Completed-test KV payloads are removed on both Macs; reports and apps remain. Continue remaining coverage in PLAN. Reports: reports/kanban-decode-integrated-r1, reports/kanban-decode-fixed-prefill-r1 and reports/kanban-echo-mtp-r1.
...
| Qwen3.8-27B inference performance | Phase 6/18 remain open; v0.4.0 is released. **2026-09-18:** the 11.99 GB q4b packed-Trellis quant decodes at 20.24 tok/s with its frozen-64 score held at 61/64 (18.92 before) on the M4 Pro 24 GB: purpose-built graph selected by quant fingerprint, device-first speculative round, 64k FR-Spec draft vocabulary, two bit-identical kernel substitutions (decode-speed-program §7.16, benchmarks). It passes every development gate (under 12 GB, 61/64, decode above the 19.63 floor; end-to-end 19.76). PR #53 carries it for v0.6.0; the exact-release full-benchmark campaign (screen, core 464, full 2,402, MMLU 1,000; about 53 GPU-hours) is prepared in `reports/qwen38-trellis-publication/release-q4b/`, handoff in its `HANDOFF.md`. Next lever after that: MTP-head adaptation to the quantized target. Fresh packed baseline and all 31 experiment classifications are recorded in decode-speed-program §7.15 and benchmarks. | **Geometry investigation complete:** down-projection 256-thread full-model identity passes but repeated timing loses; 64/160-thread controls establish no useful complete-MLP gain. Retain 128 threads. **Continued program:** direct-prefill tiles and residual/norm fusion establish no useful dependent-stage gain; mixed packing fails exactness. Bounded sampler history and exact indexed n-gram lookup are implemented with native and serial/shared HTTP identity evidence; no serving speed gain is claimed. **Next:** broaden the exact mixed evaluation-interval candidate across context/pressure and HTTP, then implement cache-owned suffix admission. Further packed work needs a distinct work-assignment/resource-lifetime candidate. Compilation, mixed MLP work and paging remain open. **Current milestone:** keep shared batching as the default with effectively equal measured B1 performance. Serving already defaults to cap eight and uses B1 for a lone eligible request. Explicit `--batch 1` retains serial; removal and complete feature parity are deferred follow-ups, not blockers for this default. **Integrated:** shared ordinary and speculative methods, sampling/lifecycle ports, generated RAM snapshots and queued SSD persistence, affine/TurboQuant row layouts including delayed precision, paged storage, adapters with lookup, and independent greedy verification sampling. Main suites, typechecks and full hygiene pass. **Current:** shared ordinary interrupted-generation resume is integrated, including queued persistence and affine/TurboQuant layouts. Both-machine native/HTTP compiled, uncompiled, delayed-precision and mixed-grammar controls, complete suites/SSD/types and the matched M4 timing comparisons are complete. The source-only timing comparison preserves every response/usage record with effectively flat performance. The packed-model HTTP fixture now gives each fresh model its own weight handles. Adapter resume and tiny/oracle-tested GLM shared MTP are also integrated from the remaining campaign checkouts. [PR #48](https://github.com/joshuarossi/mlx-bun/pull/48) contains the accumulated implementation, tests and evidence. All 80 secondary Git worktrees and 99 campaign source copies are archived and removed; both primary checkouts remain. Local typechecks, hygiene and focused integration checks pass; The implementation CI passed and v0.4.0 is published; remaining work follows the revised milestone. Explicit serial remains supported. **Remaining:** close specific unsupported media/fill/grammar-jump and adapter-resume combinations, resolve recorded B1/prefill/cache regressions, consolidate defaults/configuration, complete remaining kernel and long-task work. Serial removal is deferred. Existing provider/KV/paging correctness evidence remains accepted; umbrella L2/Lab entries need reconciliation with that evidence. Final GLM artifact testing remains deferred. No DSpark/DFlash checkpoint has been trained yet; only seeded execution fixtures exist. Feature evidence: PLAN Phase 6/18 and batching.md; performance: benchmarks.md. | feature/perf/qwen-27b-closeout |
