...
| Interface-based engine refactor | R0 fixes and initial R1/R2: portable bounded sessions, text bridge, exact-profile implementation selection, and a replaceable MLX AR binding covering graph/cache/media/decoder operations. Dedicated and generated Gemma bindings pass the same-session native gate. | Complete method bindings and feature-composition gates, then state ownership/identity migration. Use available fixtures/models for compatibility; target validation follows separately. Qwen prefix diagnosis and quiet baseline remain open. See engine architecture §12. | refactor/interface-engine-v2 |
