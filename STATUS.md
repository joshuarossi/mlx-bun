...
| Interface-based engine refactor | R1 complete; R2 bindings and shared R5 prefill pass native replacement/batch gates. Pi/web protocols are portable; request/state-view ownership is explicit. AR/spec methods share sessions and Qwen recurrent rollback gates pass. | Complete method bindings and feature-composition gates, then state ownership/identity migration. Use available fixtures/models for compatibility; target validation follows separately. Qwen replay reports Metal OOM; reconcile Josh’s other-machine fix. Quiet baseline remains open. See engine architecture §12. | refactor/interface-engine-v2 |
