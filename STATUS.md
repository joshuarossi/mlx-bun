...
| Interface-based engine refactor | R0 fixes and initial R1/R2: pinned toolchain, checkpoint stop identity, queue/prefill cancellation, portable bounded sessions, legacy text bridge, MLX graph binding, and exact-profile selection of engine-owned implementations at construction. Full migration remains open. | Complete graph/method bindings and feature-composition gates. Private target artifacts defer target validation only; refactoring continues. Qwen prefix diagnosis and quiet baseline remain validation work. See engine architecture §12. | refactor/interface-engine-v2 |
