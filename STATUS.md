...
| Interface-based engine refactor | Portable sessions, real AR/spec/denoising bindings, shared prefill, state codecs/adapter identity, resolved composition, bounded admission, client/task ports and host lifecycle implemented. Native compatibility and binary smoke pass. | Finish model-owned composition and R10 cutover. Available expanded native matrix, binary smoke and opt-in host transport checks pass. The diagnostic long-context slowdown did not reproduce in the paired repeat; quiet performance validation remains open. Quiet preflight still fails; exact target quants and second-machine results remain unavailable. See engine architecture §12.13. | refactor/interface-engine-v2 |
