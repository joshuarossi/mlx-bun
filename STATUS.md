...
|---|---|---|---|
| Repo taming (docs/files/agent instructions/seams) | Phases 0–3 landed on this branch: −50k lines, one canonical doc per topic, generated doc map, gates for every rule (`bun scripts/check-hygiene.ts`, `tests/docs-surface.test.ts`) | Phase 4 remaining: unify the two prefill loops, extract `handleChat`/model-host from server.ts, `src/lab/` quarantine + import lint; darlings are quarantined, not killed (D6). Landed: required `Cache.signature()`, real batch-lane timing, tests in `tests/{unit,serve,using,parity,research}` with CI by directory | chore/tame-jungle |
| TurboQuant weights | Verified 2026-08-24: `src/quantize/{rotate,weight-transform}.ts` + `convert --rotate-weights` shipped in v0.2.0; artifacts exist ONLY as local hub-cache staging (`models--mjriii--Qwen3.8-27B` 17 GB, `…-TQ` 14 GB, `…-MTP-TQ-bf16` 829 MB) — none exists on Hugging Face (account `mjriii` has no repos); the 51 GB bf16 trunk is still on disk | W5 GPTQ-on-rotated, W6 (upload the artifacts — Josh's decision), then delete the bf16 trunk; frozen 6-task eval cells; quiet-box tps | main (no such branch exists; verified 2026-08-24) |
