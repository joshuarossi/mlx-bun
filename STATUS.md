...
| Interface-based engine refactor | R0 fixes and initial R1/R2 implementation: pinned toolchain, checkpoint stop identity, queue/prefill cancellation, portable bounded sessions, legacy text bridge, and MLX graph binding in serial generation. New contracts are internal; full migration remains open. | Diagnose Qwen prefix crash, capture quiet baseline, complete real graph/method replacement and feature-composition gates. See engine architecture §12 for implemented scope and remaining work. | refactor/interface-engine-v2 |
