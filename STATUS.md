...
| TurboQuant weights — Q campaign (sub-4 bpw) | Q0 KL-vs-bf16 instrument landed; ladder measured (turboquant.md "Q campaign"): compact-norot 0.2524 but rawGSM 0/50; trellis k3 rot 0.4240; rotation helps trellis, hurts affine. LDLQ Hessians done, encode paused at 72/192 (Josh: "pause everything", 2026-09-01). k-map driver landed 2026-09-01. bf16 source, Q artifacts, Hessians are on the external `/Volumes/MLX-Models` volume — DETACHED as of 2026-09-01; local hub cache holds only the 08-18 flagship/compact staging | Reattach the volume; resume Q2c encode from the Hessians (or rerun 35 min), score; run Q3 (`--ldlq … --k-map trellis-kmap.json --k-budget 3.00`); task columns on every trellis arm | chore/tame-jungle |
| K3 token fast-forwarding (fill) | K3a–d landed, default off (`MLX_BUN_FILL=strict\|echo`): strict rows token-identical on the 0.8B (12/28 tokens injected, decode 15 vs 28); replay/A-B harness `scripts/fill.ts`; showcase fixture `fixtures/showcase-silicon-exchange.txt` | Live A/B needs Josh to start two servers (fill off / `MLX_BUN_FILL=echo`) on the 27B; then the showcase run | chore/tame-jungle |
