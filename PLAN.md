...
| Deferred serving inputs: engine admission, media preprocessing, adapter defaults, shutdown timeout | done (`artifacts/expert-offload` incl. the migrated builder, adapters, media input) | done for `--memory-budget` (load, admission, allocator), `--context-length`, `--force-wire`, `--allow-private-media`, `--expert-offload`, `--hlg-*`, `MLX_BUN_SHUTDOWN_TIMEOUT_MS`; missing startup adapter mounting | done (`/stats.admission.memory_budget_bytes`) | done except `--adapter`, `--adapter-path` | n/a | n/a | Adapters change: mount at startup as the default adapter with main's failure semantics |
...
| Training CLI | done (training package incl. fuse, merge, export; dropped parity tests tracked above) | done (finetune job, merge and export routes) | done | done (`train`, `train-watch`, `fuse`) | done (finetune panel) | done for short MiniCPM SFT/DPO/ORPO; other paths tracked above | Thin verbs over the job host and training API |
| Quantize CLI | done | done (quantize job) | done | done (`convert`) | done | done for uniform and mixed MiniCPM; rotation and Trellis unverified | `convert` verb over the same producer |
| Publishing CLI | done (hub upload) | n/a | done (push routes, HF settings) | done (`upload`) | done | n/a | Verb over the app publishing orchestration |
