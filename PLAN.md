...
- [~] **Kanban task acceptance:** Prior fresh tasks on the required 12 GB artifact complete with 16/18 browser categories. The integrated `721ac4c` task completes with durable output caching but falls to 10/18. A four-arm replay isolates the first-response divergence to automatic versus fixed-256 prefill; early output preserves responses within each setting. The full fixed-256 control is running on the integrated engine with the original prompt, sampling, Pi profile, empty workspace and cache capacities. Finish independent browser acceptance of the untouched output, compare matched inputs and decide the prefill profile from that evidence. Keep source and configuration changes separate from task-time claims; preserve failures and raw lifecycle records. Complete the portable Bun benchmark and HTML report at the accepted revision. Protocol: decode-speed-program §7.7; results and limits: benchmarks.md.
