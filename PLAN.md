...
| Memory synthesis (nightly pipeline) | n/a (app-owned; main's pipeline/stages/cluster/db/synthesize/wikify/crosslink ported op-for-op; `schedule` pending) | done (memory-owned `MemoryCompletionClient`; the loopback implementation posts to the server's `/v1/chat/completions`, no serial lane) | done (`GET /v1/memory/synthesize` SSE) | partial (`memory` verb: status/read/search/synthesize/stage workers/link; `init`, `setup`, `schedule`, `unschedule` pending) | read panel done; no synthesize control exists (main had none either) | n/a (main's model-free memory tests ported; the oracle/golden suites stay outside) | Stacked branch `feat/memory-setup`: `schedule.ts` with an injected home/launchctl seam, `init`/`setup`/`schedule`/`unschedule`, and `scheduleStatus` in `memory status`, the `memory_status` tool, and `/api/memory/status`. Improvement (owner: memory): the nightly launchd job runs `mlx-bun memory synthesize`, which now needs a serving mlx-bun at 03:00; the job should start (or be scheduled by) the server. Improvement (owner: memory): the entity seed gold is personal data read from a repository path; seed from the vault's own aliases (`EntityResolver.fromStore`) and drop the file. Improvement (owner: memory): `--since`/`--model` are parsed but never consumed; consume them (`model` maps naturally to the served model id) or remove them. |
| Live model switching, isolation, model pool | n/a | partial (worker mode: `cli/worker-entry.ts`, `jobs/worker-process.ts`; main's parent proxy, `parent-application`, `responses-client` missing) | restart answer done (`/api/hub/serve`); worker socket serves `/health`, `/admin/lease`, `/admin/drain` (`server/worker-routes.ts`); 501 on TCP (`/admin/lease`, `/admin/drain`, `/engine`) | flags absent | hub panel done | n/a | Decided (Josh): isolation and pooling stay as optional capabilities, off by default with a pool cap of 1, and web chat works under isolation (main answered 501 there). Owning implementation, after the CLI batch: I1 done: `apps/mlx-bun/src/cli/serve-state.ts` (persistent CPU state, no engine or native import) and `serve-host.ts` (model-scoped host borrowing it by parameter) behind an unchanged `startModelServer`, proven by `tests/serve-composition.test.ts`; I2 done: worker mode (`cli/worker-entry.ts` over `startModelHost` with the internal `unix`/`routes` hooks, `server/worker-routes.ts`, `jobs/worker-process.ts` re-executing through `jobs/executable.ts` as `__worker`), proven by `tests/worker-entry.test.ts`, `tests/jobs/worker-process.test.ts`, `tests/server/worker-routes.test.ts`; I3 adds the parent proxy behind `--isolate` (restart budget, 502 `engine_unavailable`, drain, shutdown, parent-owned Pi over loopback HTTP and parent-owned Responses history, `/engine`) with browser stream/stop/crash/reconnect acceptance in the same change; I4 adds the exact-id LRU pool behind `--model-pool` with serialized cold starts, drained eviction, library invalidation reaching live hosts, GC protecting every resident or loading model, Pi `local` as the default-worker alias, and job leases spanning all workers until child exit and log drain. Keeps the existing HTTP boundary; tensor-bearing engine calls are never transported. |
...
| Serial-only request shapes: Gemma2 advanced compositions (ordinary plain KV now batches), sliding-attention caches, media without batched input, non-batchable quantized and Turbo KV, grammar with batching off, DiffusionGemma denoising | done (models exist, incl. `diffusion-gemma`, `universal`) | typed `UnsupportedExecutionError` per shape | 501 envelope | `generate` prints the error, exit 1 | n/a | per shape, when supported | Shared-scheduler support per shape (to support, not dropped); DiffusionGemma needs a denoising plan in the batched executor; resolve `--kv-quant turbo`, `--fused-sdpa`, `--l1`/`--l2`/`--l3` with layer policy |
