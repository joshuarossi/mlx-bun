...
- [ ] **Completed default-suite follow-up:** resolve MiniCPM/Qwen timed-output divergence against the matched reference, Qwen 4/8-bit 1K/context Metal failures, and MiniCPM/Gemma12B first-attempt SSD snapshot flush failures. Re-run affected exact requests before accepting those cells; record the required packed Trellis/KV4/MTP configuration separately. Evidence and qualified numbers: benchmarks.md, standard M4 Pro matrix.
