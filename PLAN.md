...
- [x] **Configuration consolidation:** complete. Bound settings, owner inventory, compatible controls and accepted measurements are recorded in architecture §12.5–12.7 and §12.14, server-config.md and benchmarks.md.
- [x] **Measured defaults:** v0.4.0 selects Trellis v13, eligible Qwen KV4 speculation, paired MTP prefill reuse, shared batching, R17 persistence, fused TurboQuant decoding and automatic prefill sizing without opt-in flags; KV4/MTP2 is the measured long-agent recommendation. Defaults: server-config.md; evidence: benchmarks.md.
...
- [~] **27B R1–R7:** variants 7/8/10 share small-M gate/up and scatter work. Variants 11/12 add direct short-prefill tiles, with native/HTTP state and response acceptance. The existing single-request suite confirms the v12/v6 decode improvement in two AB/BA pairs. Forty larger fused-tile cases are exact but slower. Variant 13 instead vectorizes bf16 expansion; operation, full-model and HTTP outputs are exact. Smaller native, serial HTTP and default-scheduler prefills improve, while existing-suite HTTP timing is mostly flat. R6 integrated readers and a lossless separate artifact preserve native generation and both HTTP paths with a small decode gain, flat RSS and no duplicate codes. Its six-pair cache-enabled default-shared KV4/MTP2 acceptance now passes with exact responses/usage and durable SSD restart; every pair improves decode and cached completion. Bounded v13 expansion scheduling is opt-in after native, both short HTTP repeats and both long-agent pressure gates pass; the combined fused-k8v3/MTP3 pressure fixture now also passes on M4 after M1 acceptance. Six M4 Pro serial v6/v13 pairs now pass under the recorded audio-CPU allowance, with exact outputs and a fixed source; final-source native state and bf16/KV4 MTP-prefix gates also pass. Six continuous and six combined KV4/MTP pairs pass with matching outputs; the bounded combined expansion/TQ/MTP pressure gate is now accepted. Variant 9 and the numerical M<=8 crossover remain Lab. Audit the packed activation distinction separately.
- [~] **27B R15:** Parser guards and model-owned committed-token appends are integrated. Native affine row arithmetic and MLX attention-length transitions preserve one-token logits/state on qualified M4 Pro Qwen graphs. Operation, full-model boundary, model-free, typecheck and HTTP cancellation gates pass. The final internal-SSD comparison completes 96 exact responses across twelve balanced pairs with request-time gains on both quants and no failures. Both earlier external-drive warmup failures remain recorded separately; active models now use verified internal copies. Speculative combinations remain gated. Reusing the existing state-view interface fixes retained append views; KV4/KV8 retain one-token attention inside committed appends, and K8V3 uses the same model-owned format declaration and delayed-conversion boundary. Both packed/RTN4 native gates and six-pair HTTP comparisons pass with exact outputs and request-time gains; saved tool-history fixtures retain all 20 affine pairs. Shared strict fill now reuses the prefill/row/sampler/cache interfaces and preserves the sampled-token pipeline; B1/B2/B4 seeded replay passes on both Macs across covered bf16/KV4/k8v3 models. Qualified B1 multi-position appends now pass hidden/cache/logit and delayed-precision gates. HTTP/cache acceptance completes all 108 responses and six SSD restarts with exact paired outputs/counts/cache coverage. Shared strict fill improves the sequential fixture total by 13.50%/13.57% against fill-off and stays within 0.13% of serial; concurrent latency tradeoffs remain. Shared echo now composes the existing grouped verifier and output-aligned row rollback; M1 Qwen/Gemma and M4 packed-Qwen sampling/state/SSD gates pass. Actual Qwen/Gemma HTTP copied arguments and restart checks pass. M4 strict/echo ABBA is repeated after competing browser/test activity: all 38 measured response/usage pairs match; sentence/URL copies improve about 12%/41% in both orders. Mixed ordinary/echo cohorts and independent greedy sampling pass both-machine correctness and M4 ABBA; retain the opt-in method with its documented concurrent latency tradeoff. Qwen echo+MTP now passes companion oracle, shared sampling/state and real HTTP/SSD checks on both Macs. M4 ABBA preserves all 38 measured pairs with copy-length-dependent gains and losses. The requested full MTP2+echo Kanban task completes with 17/18 unchanged-app acceptance and all follow-ups hitting the session cache; its streaming rate is flat, with more output and model-script failures increasing task time. Keep echo opt-in. Fill remains opt-in. Held-out, other-model, combined/pressure and quiet gates remain. Evidence: speculative-decoding §7.4.
- [~] **27B R8–R14/R16:** Shared affine weight reuse improves RTN4 M3..8 and compact M4/8 full forwards and serving with exact outputs; M2 and unsupported head cells stay native. Final flag pairs, shapeless fallback, all typechecks and the complete model-free tier pass. Compiled-call caching is exact but its small native gain disappears in HTTP; no integration. A broader 4-bit prototype preserves full-model state on Gemma 12B/e4b, MiniCPM and Llama 1B/3B; only the larger Gemma cells improve. The narrower integrated selection passes native/live-input checks and Gemma HTTP timing pairs against both prior shared dispatch and native. B4/8 improve, B1 is flat; recorded B8 warmup response variation remains. Finish broader-model controls. Compact state copy passes packed gates; fused convolution, GDN fusion and affine layout screens have no compelling model win. Shared grammar masking preserves HTTP responses but timing is inconclusive. D256 attention fails one KL screen. Early first output is adopted after native and sampled Q3/RTN4 gates. Continuous scheduling passes integrated HTTP acceptance after six exact prototype pairs; native Q3/RTN4 cache reuse and both long cached/SSD gates pass. Actual HTTP cancellation, independent recovery and overlapping arrival now pass on both Qwen quants and serving lanes. The caller-owned first-token cache boundary is reconciled: explicit M=1 alignment preserves full state and all five continuation shapes on both M4 quants, alongside existing M1 gates. Monitored six-pair serving comparisons support the new default with complete-time tradeoffs recorded. Continue the shared-operation audit: M4 three-head KV4 key grouping now passes operation, full-state and six-pair HTTP gates and is selected for the qualified geometry. The existing wide-prefill kernel fails M1 exactness and remains M3+. Fixed-arrival two/four/eight-wave diagnostics preserve every paired response and favor default aggregate throughput, while first-request/stream-pause tradeoffs remain. Smaller chunks and unpacked mixed work lose complete time; their defaults stay unchanged.
- [~] **27B R17–R24:** R24 now has per-candidate resource/kernel requirements and a rejected native activation-quantization screen; simple actual-weight low-rank/residual variants are rejected and packed differentiation needs a custom VJP; calibrated quality/training remain open. R19 now has both-machine k2/k3/k4 role costs and 64 size-constrained MLP allocation proposals; no candidate is promoted because quality is unmeasured and estimated decode headroom is small. Media encoder and generated media-prefix RAM/SSD acceptance are complete; prepared media KV reuse is selected by default after both-machine correctness and M4 comparisons. Native MTP cost recheck finds workload-dependent results. Six three-arm held-out HTTP blocks on the interleaved packed artifact preserve response text and improve complete time for fixed/adaptive MTP, with additional RSS. The native follow-up preserves token IDs and source-cache alignment through 500 rounds; EOS usage now follows native/oracle accounting. Served EOS-at-budget checks pass. Six saved-companion blocks preserve all responses with modest request-time gains and lower RSS; native same-input logit/state gates pass; six matched-prefill blocks per packed/RTN4 quant preserve all MTP/native responses and median complete-time improvements, with known RTN4 tail-split differences retained separately. Longer sessions, other quants and quiet gates remain. Cache/scheduler, all quants, wider algorithms and combined frontier on the M4 Pro remain open.
- [x] **R14 normalized greedy selection:** one sampler-owned tiled kernel avoids the normalized vocabulary output for ordinary and independent verification sampling. Same-input native model/operation checks pass on both Macs, including dtype ties, layouts, grammar and metadata exclusions. Standard-script AB/BA pairs improve MiniCPM with Qwen MTP3/TQ effectively flat; response differences and limits are recorded in benchmarks.md.
- [~] **Project-wide kernel and graph coverage:** the fresh 12.14 GiB packed-Qwen GPU/memory baseline and implementation/method review are recorded in [decode-speed-program §7.15](docs/design/decode-speed-program.md#715-implementation-and-measurement-review); earlier negative outcomes do not close an idea. The first gate/up spill-shuffle experiment passes exactness and activation but establishes no useful local/native gain; retain the incumbent. The down-projection geometry investigation is complete: exact 256-thread full-model gates pass, but repeated component timings lose; 64 threads loses at M3/M4 and 160 threads establishes no repeatable complete-MLP gain. Retain 128 threads. Further packed work needs a distinct work-assignment or resource-lifetime candidate. Current-artifact direct-prefill tiles lose complete-MLP timing; mixed packing violates exactness, so retain its geometry guard. Residual/norm fusion establishes no dependent-stage gain. Bounded sampler history and exact indexed n-gram lookup are implemented with component/native and serial/shared HTTP identity evidence; no serving speed gain is claimed. Wider mixed evaluation intervals remain experimental pending broader context/pressure and HTTP gates. Cache-resolved admission needs owned preparation state before policy changes. Inventory every owned Metal kernel, invoked native MLX operation, compiled graph and fallback; map each to model families and generating/serving workloads. Retain improvements only after complete native and serial/continuous HTTP comparisons; inspect scheduling, synchronization, buffer lifetimes and next-step overlap when isolated fusion gains disappear. Both sparse-attention keys now use uint32; exact operation/model-state gates and balanced selection timing pass, while real Colibri serving remains unmeasured. Top-p fusion preserves full requests but fails the Qwen timing repeat. Submission/traversal screens retain no Qwen setting, and omitting untouched finalizations also fails its balanced repeat. The current MLX 0.32.2 submission screen, both-machine native identity and broader-context/SSD controls are complete. Retain native defaults after mixed cross-model results; M4 Llama's gain remains available through explicit startup overrides. The native API has no model-scoped setter. Device Trellis codebook lookup loses and is closed. The scoped threadgroup table is integrated into experimental v13; full-model identity and six integrated MTP HTTP pairs pass with small complete-time gains. Six integrated four-request serving pairs preserve all responses with a small median gain; per-cohort call maps separate matched work from arrival scheduling. Combined/pressure and quiet acceptance remain. Include memory movement and host overhead. Qwen is the lead workload, not the scope boundary. Preserve a per-operation evidence ledger and a repeatable M4 Pro comparison bundle.
- [~] **Video follow-ups:** Joint TurboQuant K/V decoding is integrated as an opt-in after operation, long-context, RTN4 and deferred-consumer state gates. Integrated R6/RTN4 native identity and six serial HTTP pairs per quant pass response gates. Allocation diagnostics distinguish live-array ownership from MLX buffer capacity. Combined fused-k8v3/MTP3 pressure acceptance now passes on both Macs; fused decoding is selected by default after current shared-serving comparisons, with the mixed short-Qwen result retained. Six balanced long-context three-arm HTTP blocks preserve all responses and improve complete time. The narrower inverse-rotation operation passes both integrated model gates and six HTTP pairs per quant; every decode pair improves, R6 complete time improves and RTN4 complete time remains inconclusive. Grouped greedy readback is closed after six balanced three-arm HTTP blocks preserve responses but establish no broad request-time benefit. Direct packed-key attention is exact but slower than the integrated decoder in complete-operation repeats; that additional speed candidate is closed. The integrated R6 and RTN4 inverse-rotation gates pass on MLX 0.32.2. Six actual HTTP pairs each on MiniCPM/Gemma deferred consumers preserve responses and improve complete time on consolidated source. Current-artifact context/memory and Qwen DFlash2 architecture/selector work remain. Approximate prompt selection stays a separate quality-gated Lab arm. Details: decode-speed-program §7.11 and turboquant.md.
- [x] **Latest MLX runtime adoption:** MLX 0.32.2 / MLX-C in native pack 0.4.0 (macOS 14); every same-version logit/state, serving, SSD-restart and integrated Kanban gate passes; the SSD key carries the runtime/GPU numerical identity. Details: decode-speed-program §7.12; [Kanban evidence](docs/reference/benchmarks.md#full-kanban-after-decode-integration).
...
- [~] **Final benchmark and multi-machine HTML report:** offline native/serve/task import, HTML rendering, fixtures and full-output identity checks are implemented. Incompatible request, artifact, host, source and qualification cells cannot supply speed ratios; legacy failures and retries remain visible. The bounded M1 MiniCPM live smoke completes; concurrent reference output differences remain, and diagnostic timings cannot qualify. Paired block uncertainty and same-arm environment baselines are implemented and tested. Saved Pi tasks now include individual app acceptance checks, settings, per-turn timing/usage and persistence results; task time never supplies an engine-speed ratio. Portable quality-ledger import and accuracy/size plots are implemented, with legacy dataset/artifact identity limitations visible. The current M4 standard AB/BA matrix completes with fixed short/context outputs; a repeated token-history conversion cost is isolated and reduced in two controlled MiniCPM pairs. The default-on follow-up also completes with fixed short/context outputs and successful SSD restore. Complete the remaining frozen-protocol and current-artifact quality cells after optimization gates. Cover single-request latency/throughput, prefill, startup, memory, cache reuse and sustained serving with same-artifact MLX-LM controls where supported. Protocol: decode-speed-program §7.10; current renderer limits: benchmarks.md.
...
- [~] **Shared execution and feature parity:** ordinary and speculative methods
      share scheduling, sampling/seed, logprobs, row lifecycle, usage and cancellation.
      Affine/TurboQuant layouts, delayed/per-layer precision, provider-owned drafts,
      generated RAM/SSD state, paging, adapters with lookup, and ordinary/adapter
      resume are integrated. Existing native, HTTP, ownership, restart and same-B
      ...
      **Current:** Gemma media acceptance is complete with recorded latency costs. Qwen image/video acceptance also passes both-machine native/HTTP and M4 ABBA checks; Qwen encoder objects pass both-machine RAM/SSD/HTTP and M4 source-controlled timing, including the first-use hash cost. Gemma encoder caching also passes both-machine identity/restart and M4 ABBA acceptance with all 168 paired responses/usage unchanged, repeated-request improvements and first-use costs recorded. Gemma and Qwen media KV reuse now pass both-machine native and RAM/SSD HTTP checks, fresh-process restoration and M4 ABBA comparisons. Shared media-prefix reuse is selected by default; retained outputs, precision boundaries and latency effects are documented. **Remaining:** broader fill combinations,
      ...
      The shared sampler, readiness/early output, generated cache and ordinary
      resume need no repeated unchanged validation. A serial fallback does not
      qualify shared execution; strict dominance is required before deferred serial removal.
...
- [x] **Ring-wrap oracle (> sliding window):** unequal offsets, wrap during decode, retirement and late joins match the pinned same-B oracle on both machines (Gemma 12B, e4b).
- [x] **L2 same-B affine oracle:** MiniCPM full-attention and Gemma4-e4b rotating bf16/KV4/KV8/mixed same-B cases match every vector per machine, including retirement, reordering, unequal lengths and the window boundary; closes the named same-B oracle gap, not every future layout.
...
- [x] **C1–C5 cache expansion:** shared SSD blocks, async restore/prefetch, interchangeable RAM policy (LRU retained), segmented storage copies and direct paged attention (Lab/optional) landed and measured. [Evidence](docs/reference/benchmarks.md#cache-expansion-c1c5-storage-restore-retention-and-paged-attention).
...
- [x] **C6 — Session checkpoint index:** session/content indexes with soft RAM affinity compose ordinary/MTP/paged RAM/SSD reuse; throughput flat, searches/restores fall; full Kanban passes with durable flush. kv-cache §5.12, benchmarks.md.
...
## Phase: Whisper speech-to-text `[~]` (opened 2026-09-15)

Audio in → text out with `mlx-community/whisper-large-v3-turbo` (sotto backend,
TUI/web voice input). Design + evidence: [generic-model-support.md §6.7](docs/design/generic-model-support.md#67-whisper-speech-to-text--srcmodelwhisperts-srcaudiowhisper-ts-srcservetranscription-ts); oracle venv: [environment.md](docs/reference/environment.md#the-python-oracle).

- [x] **P0–P2 (2026-09-15):** loader/FFI groundwork, bit-exact mlx-whisper parity
      (mel, encoder, per-step logits; 7 cases), greedy/beam/prompt decoding, `mlx-bun transcribe`,
      `/v1/audio/*` companion + transcription-only server with idle page-out, model-free + server tests.
- [x] **P3 performance (2026-09-15, M1 Max):** fast path (fused SDPA, fused
      cross-K/V and encoder q/k/v, compiled decoder step with device-side
      filters, pipelined greedy, in-graph beam top-k, word timestamps) gated
      at ≤ 2 differing tokens per clip; 8 % faster than mlx-whisper greedy,
      11–15 % faster than whisper.cpp beam-5. Dead ends and the Lab
      `audio_ctx` trade are recorded in [benchmarks.md](docs/reference/benchmarks.md#whisper-speech-to-text-head-to-head--m1-max-32-gb-2026-09-15).
- [x] **P4 dictation path (2026-09-16):** Silero VAD v6.2 port (gate + in-stream,
      pip-reference parity), streaming sessions (a 62 s take finishes 327 ms
      after its last chunk), AudioToolbox in-process decode (3.7 vs 72 ms),
      Core ML/ANE encoder measured and shelved (940 vs 288 ms on the M1 Max).
- [ ] **Open:** M4 Pro cells; `bench-serve` cell; Lab 4/8-bit weights and energy-split
      chunking; page-in compile warm-up; on-device VAD recurrence for >60 s; ANE-shaped
      encoder for weak-GPU Macs. `mlx-bun dictate` is the push-to-talk prototype; the sotto PR and web/TUI voice input follow it.
...
- [ ] **Packed Qwen publication:** [provenance, exact-release benchmarks, model card and clean-install acceptance](docs/design/turboquant.md#packed-qwen-publication-preparation). Development gate met 2026-09-18: the 11.99 GB q4b artifact holds 61/64 on the frozen screen at 20.24 decode tok/s on the M4 Pro 24 GB (was 18.92) through its purpose-built graph, the device-first round and the 64k draft vocabulary ([decode-speed-program §7.16](docs/design/decode-speed-program.md)); the full publication suite is still owed.
