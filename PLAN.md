...
Evidence means paired real-weight preservation against main (and the oracle
where one exists); synthetic tests never fill it. Serial-only and compilation
switches stay absent by Josh's decision; nothing else here is dropped without one.
...
| Evaluation and benchmarking: `bench`, `evals`, `perplexity`, EvalDB, eval tasks | partial (`scoring/perplexity`, diagnostic `scripts/bench.ts`) | n/a | `/fit` measured fields null (EvalDB absent) | missing | status page shows dashes | n/a | Josh decides the eval-tooling scope; the performance pass needs at least `bench` |
...
| Lab pages: `/curves`, `/curve-terrain`, `/dag`, `/generate`, `/signal`, expert-trace, paged-kv lab | n/a | n/a | 501 | n/a | `/dag` probe degrades | n/a | Josh decides keep-as-lab (port `src/lab/*` under the app) or drop; not product surface |
...
| Docs surface and gates | n/a | n/a | served surface documented only in `--help` and READMEs | same | same | n/a | Recreate main's `docs/reference/*` homes (cli, server-api, server-config, models, benchmarks, environment, distribution, training, memory) with the surface and hygiene gates; numbers land only in benchmarks |
