...
| Live model switching, isolation, model pool | n/a | partial (worker mode: `cli/worker-entry.ts`, `jobs/worker-process.ts`; parent proxy: `cli/serve-isolated.ts`, `jobs/worker-supervisor.ts`; model pool missing) | restart answer done (`/api/hub/serve`); worker socket serves `/health`, `/admin/lease`, `/admin/drain` (`server/worker-routes.ts`); under `--isolate` the parent serves `/engine`, `/health`, `/stats`, `/downloads`, `/v1/responses` history (`server/proxy-routes.ts`, `server/responses-client.ts`) and proxies the rest; 501 on TCP (`/admin/lease`, `/admin/drain`; `/engine` without `--isolate`) | `--isolate` done; `--model-pool` absent | hub panel done | opt-in `tests/engine/isolate.test.ts` (real worker kill and respawn; not yet run) | Decided (Josh): isolation and pooling stay as optional capabilities, off by default with a pool cap of 1, and web chat works under isolation (main answered 501 there). Owning implementation, after the CLI batch: I1 done: `apps/mlx-bun/src/cli/serve-state.ts` (persistent CPU state, no engine or native import) and `serve-host.ts` (model-scoped host borrowing it by parameter) behind an unchanged `startModelServer`, proven by `tests/serve-composition.test.ts`; I2 done: worker mode (`cli/worker-entry.ts` over `startModelHost` with the internal `unix`/`routes` hooks, `server/worker-routes.ts`, `jobs/worker-process.ts` re-executing through `jobs/executable.ts` as `__worker`), proven by `tests/worker-entry.test.ts`, `tests/jobs/worker-process.test.ts`, `tests/server/worker-routes.test.ts`; I3 done: the parent proxy behind `--isolate` (`cli/serve-isolated.ts` composing `createAppState` plus `server/proxy-routes.ts` over `jobs/worker-supervisor.ts`: main's restart budget, 502 `engine_unavailable` with the protocol's SSE error frame, drain before stop, parent-owned Pi over loopback HTTP and parent-owned Responses history in `server/responses-client.ts`, `/engine`), documented in the [app README](apps/mlx-bun/README.md#runtime-isolation---isolate) and proven by `tests/serve-isolated.test.ts`, `tests/jobs/worker-supervisor.test.ts`, `tests/server/proxy-routes.test.ts`; browser stream/stop/crash/reconnect acceptance is the reviewer's; I4 adds the exact-id LRU pool behind `--model-pool` with serialized cold starts, drained eviction, library invalidation reaching live hosts, GC protecting every resident or loading model, Pi `local` as the default-worker alias, and job leases spanning all workers until child exit and log drain. Keeps the existing HTTP boundary; tensor-bearing engine calls are never transported. |
