...
- [~] **Shared execution and feature parity:** ordinary and speculative methods
      ...
      **Current:** Gemma media acceptance is complete with recorded latency costs. Qwen image/video acceptance also passes both-machine native/HTTP and M4 ABBA checks; Qwen encoder objects pass both-machine RAM/SSD/HTTP and M4 source-controlled timing, including the first-use hash cost. Gemma encoder caching also passes both-machine identity/restart and M4 ABBA acceptance with all 168 paired responses/usage unchanged, repeated-request improvements and first-use costs recorded. Gemma media KV reuse is implemented behind an experimental option; both-machine native state and RAM/SSD HTTP checks pass, including precision boundaries. M4 ABBA improves all scenarios with one recorded punctuation difference. Qwen media KV is integrated through the same identity/input ports; both-machine native and actual packed27B RAM/SSD/image/video acceptance passes. Measure its M4 session benefit before default selection. **Remaining:** broader fill combinations,
