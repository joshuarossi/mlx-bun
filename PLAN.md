...
- **temp>0 is first-class** (lossless speculative SAMPLING: min(1,p/q) accept +
  ...
  proven statistically in `dspark-smoke.ts (deleted 2026-08-23; git history)` (maxErr<0.01 over 200k
...
- [x] **14g — MTP speculation: DeltaNet rollback SOLVED (2026-08-18); real-
      ...
      **All follow-ups closed (2026-08-18, quiet M1 Max):**
      (1) QUIET-MACHINE TPS A/B (qwen38-mtp-ab.ts (deleted 2026-08-23; git history),
      ...
      **MTP-on is 0.821× at γ=2 despite 61% acceptance and 2.25
      ...
      0.6.14 reference (scripts/oracle/oracle-qwen38-mtp-logits.py +
...
- [x] **P2R8 — report causes, not just totals.** Produce an additive table and
      ...
      record; promote quiet-machine conclusions to `docs/reference/benchmarks.md` and
      ...
      `scripts/render-p2r-html.ts` from the 1,290-row JSONL source
...
- [ ] **E0 — per-task expert-skew measurement** (make-or-break, pure
      ...
      + `analyze-expert-trace.ts (deleted 2026-08-23; git history)` (coverage / working-set / cold-
      ...
      run-expert-trace.ts (deleted 2026-08-23; git history) over 8 prompts × 3 domains, 26B): gate
...
- [~] **E1 — offload-ready file + mmap expert loading** behind
   ...
   - [x] **E1a — converter DONE 2026-06-14**: `convert-offload-experts.ts (deleted 2026-08-23; git history)`
...
frozen 25-case holdout. Phase 0 smoke (`ft-chunk-smoke.ts (deleted 2026-08-23; git history)`)
...
- [x] **W0 folding spike (small model)** — DONE 2026-08-17 (branch
      ...
      existing writer) + `scripts/turboquant/w0-fold-llama.ts` (runner
...
- [~] **W5 calibration composition (the 4-bit flagship win)** — OPENED
      ...
      (forked minimally in scripts/turboquant/tq-gptq.py: language-only
...
flank.) Method: interleaved fresh-process pairs via `bench.ts (deleted 2026-08-23; git history)` /
`--baseline` (bf16 KV, identical filler+template conventions, medians),
plus in-process sweeps (`prefill-chunk-ab.ts (deleted 2026-08-23; git history)`), a
weight-residency A/B, logits-level chunk-size probes
(`prefill-logits-ab.ts (deleted 2026-08-23; git history)` + python twin), and one served
