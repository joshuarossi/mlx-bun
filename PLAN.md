...
- [x] **Measured defaults:** v0.4.0 selects Trellis v13, eligible
  Qwen KV4 speculation and paired MTP prefill reuse without opt-in flags. Shared
  batching is the serving default and R17 output persistence is complete; fused TurboQuant decoding now defaults on after shared-serving and combined-pressure acceptance.
  Setting selection uses the recorded comparison of MTP
  depths 1/2/3/4 and ordinary decode: the short screen and four context repeats
  are complete. Late-context 2048-token prefill failed; 256-token chunks complete at 78,678 prompt tokens with MTP1/2/3; the original ordinary cached-repeat workload now passes on current cache code, with the exact first-attempt request and response, two successful repeats and durable SSD restart. The original failure and changed retry remain recorded. Long-context TurboQuant/MTP3 also passes; the current-source KV4/MTP2 setting comparison also passes. KV4/MTP2 remains the measured long-agent recommendation and MTP2 is already the companion default. Automatic prefill sizing is adopted after exact response/count acceptance at 78,678 tokens, cached repeats and SSD restart; explicit overrides remain supported. Defaults: server-config.md; evidence: benchmarks.md.
...
- [x] **L2 same-B affine oracle:** existing mixed-KV B1 logits match the
      ...
      and reordering. Unequal-length full-attention bf16/KV4/KV8/mixed cases now match all 24 vectors per machine through a documented test-only composition of pinned storage, positions and masking; stock mlx-lm lacks this cache. Gemma4-e4b rotating bf16/KV4/KV8/mixed cases also match 48 equal-length and 24 unequal-length vectors per machine across the actual window boundary and retirement. Existing transition/provider tests remain accepted; this closes the named same-B affine/rotating oracle gap, not every future layout or arrival pattern.
