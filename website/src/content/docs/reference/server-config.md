...
description: serve flags, environment variables, and the --batch compatibility matrix.
---
...
adapters admin) see [server-api.md](/reference/server-api/); this doc is about
*starting* the server.

The flags are parsed in `serverRuntimeFlags()`
([src/cli.ts](https://github.com/joshuarossi/mlx-bun/blob/main/src/cli.ts)); the runtime knobs live in
`ServerOptions` ([src/server.ts](https://github.com/joshuarossi/mlx-bun/blob/main/src/server.ts)). The two execution
lanes (serial vs. `--batch N`) and the lane picker are in
[src/serve/generation-gateway.ts](https://github.com/joshuarossi/mlx-bun/blob/main/src/serve/generation-gateway.ts);
the design rationale is in
[docs/design/parallel-slots.md](https://github.com/joshuarossi/mlx-bun/blob/main/docs/design/parallel-slots.md).
...
| `--hlg-sampling` | `on`\|`off` | off | Piecewise tone-curve (HLG) sampling: rolls off the top-token region, boosts the mids, gentles the tail. The overall gain folds from `--temperature`. See [docs/design/hlg-sampling.md](https://github.com/joshuarossi/mlx-bun/blob/main/docs/design/hlg-sampling.md). |
...
`handleChat`. Full field list: [server-api.md](/reference/server-api/).
...
([batch-scheduler.ts](https://github.com/joshuarossi/mlx-bun/blob/main/src/serve/batch-scheduler.ts)) drives the
...
- **`--perf-kernel` / `--fused-decode` / `--compiled-decode`** — never
  ...
  [parallel-slots.md](https://github.com/joshuarossi/mlx-bun/blob/main/docs/design/parallel-slots.md).
