...
import { MlxArray as Arr, gpuStream, type MlxArray } from "../mlx/array";
import { Dtype, clearCache, activeMemory, cacheMemory, peakMemory } from "../mlx/ffi";
...
export class Trellis {
  ...
  /** Trellis-encode [B,T] rows (already variance-matched) and return the
   *  decoded STATE at every t as [B,T] int32 — the k low bits of state t are
   *  the emitted symbol, and (bitshift trellis) state t is exactly the last
   *  L/k symbols, which is what makes random-access decode possible. */
  encodeStates(X: MlxArray): MlxArray {
    const [B, T] = X.shape as [number, number];
    if (T !== this.cfg.T) throw new Error(`block length ${T} != cfg.T ${this.cfg.T}`);
    const two = ops.mulScalar(X, 2, S);
    const twoT = ops.transposeAxes(two, [1, 0], S);
    const X2T = ops.contiguous(twoT, S);
    two.dispose(); twoT.dispose();
    ops.evalAll([X2T]);
    let overlap: MlxArray | null = null;
    if (this.cfg.tailBiting) {
    ...
    X2T.dispose();
    overlap?.dispose();
    const idx = ops.concatAxis(st as MlxArray[], 1, S);
    for (const s of st) s?.dispose();
    ops.evalAll([idx]);
    return idx;
  }

  /** States → decoded values ([B,T] f32), the exact map encodeDecode uses. */
  decodeStates(idx: MlxArray): MlxArray {
    const out = ops.takeAxis(this.lutFlat, idx, 0, S);
    ops.evalAll([out]);
    return out;
  }

  /** fakeQuantRows that ALSO returns the packed bit-stream and the fp16 row
   *  scales — the on-disk trellis format (see packStates). `rec` is the same
   *  f32 reconstruction fakeQuantRows returns, so the fake-quant and packed
   *  artifacts are built from one Viterbi pass. */
  fakeQuantRowsPacked(
    W: MlxArray, batchBlocks: number, onProgress?: (done: number, total: number) => void,
  ): { rec: MlxArray; codes: MlxArray; scales: MlxArray } {
    const [N, C] = W.shape as [number, number];
    const T = this.cfg.T;
    if (C % T !== 0) throw new Error(`trellis axis ${C} not a multiple of block ${T}`);
    const sq = ops.square(W, S);
    ...
    const rms = ops.sqrt(ms, S);
    ms.dispose();
    const inv = ops.mulScalar(rms, 1 / this.lutRms, S);
    rms.dispose();
    const zero = Arr.fromFloat32(new Float32Array([0]), []);
    const one = Arr.fromFloat32(new Float32Array([1]), []);
    const isz = ops.equal(inv, zero, S);
    const scale = ops.where(isz, one, inv, S);
    for (const a of [zero, one, isz, inv]) a.dispose();
    const s16 = scale.astype(Dtype.float16, S);          // [N,1] f16 — stored
    const scaleQ = s16.astype(Dtype.float32, S);
    scale.dispose();
    const scales = ops.reshape(s16, [N], S);
    s16.dispose();
    ops.evalAll([scales]);
    ...
    const nBlocks = (N * C) / T;
    const wordsPerBlock = (T * this.cfg.K) / 32;
    const packed = new Uint32Array(nBlocks * wordsPerBlock);
    const parts: MlxArray[] = [];
    for (let b0 = 0; b0 < nBlocks; b0 += batchBlocks) {
      const b1 = Math.min(nBlocks, b0 + batchBlocks);
      const sl = flat.slice([b0, 0], [b1, T], S);
      ...
      sl.dispose();
      ops.evalAll([chunk]);
      const idx = this.encodeStates(chunk);
      chunk.dispose();
      const rec = this.decodeStates(idx);
      parts.push(rec);
      const u32 = idx.astype(Dtype.uint32, S);           // rawBytes has no int32 path; states are >= 0
      const raw = u32.rawBytes();                        // [B,T] row-major
      u32.dispose();
      const st = new Int32Array(raw.buffer, raw.byteOffset, raw.byteLength / 4);
      packStates(st, b1 - b0, T, this.cfg.K, packed.subarray(b0 * wordsPerBlock, b1 * wordsPerBlock));
      idx.dispose();
      clearCache();
      onProgress?.(b1, nBlocks);
    }
    ...
    const codes = Arr.fromBytesCopy(
      ...
      [N, (C * this.cfg.K) / 32], Dtype.uint32,
    );
    return { rec: out, codes, scales };
  }
}

// ------------------------------------------------------------ packed format ---
//
// One coded row = C/T blocks; one block = T·k bits = T·k/32 uint32 words
// (little-endian words, bit i of the block = word i>>5, bit i&31). Symbol b_t
// (= state_t & (2^k−1)) sits at bit offset (T−1−t)·k — REVERSED time — so the
// L-bit window starting at that offset, read LSB-first and wrapping around the
// block (tail-biting), is exactly state_t = b_t | b_{t−1}<<k | … . The kernel
// therefore decodes any position in O(1): two word loads, a shift, a mask, and
// the 1MAD hash (or a 4096-entry LUT). The fp16 per-row scale is stored as a
// separate `.scales` [rows] tensor; the reconstructed weight is
// bf16(f32(lut[state]) · f32(scale)), bit-identical to the fake-quant artifact.
...
export function packStates(states: Int32Array, nBlocks: number, T: number, k: number, out: Uint32Array): void {
  const wpb = wordsPerBlock(T, k);
  const mask = (1 << k) - 1;
  for (let b = 0; b < nBlocks; b++) {
    const base = b * wpb;
    for (let w = 0; w < wpb; w++) out[base + w] = 0;
    for (let t = 0; t < T; t++) {
      const sym = states[b * T + t]! & mask;
      const p = (T - 1 - t) * k;
      const wi = p >>> 5, off = p & 31;
      out[base + wi] = (out[base + wi]! | (sym << off)) >>> 0;
      if (off + k > 32) out[base + wi + 1] = (out[base + wi + 1]! | (sym >>> (32 - off))) >>> 0;
    }
  }
}

/** Read state_t back out of one packed block (host reference for the kernel). */
export function stateAt(words: Uint32Array, blockWordOffset: number, wpb: number, t: number, T: number, k: number, L: number): number {
  const p = (T - 1 - t) * k;
  const wi = p >>> 5, off = p & 31;
  const w0 = words[blockWordOffset + wi]!;
  const w1 = words[blockWordOffset + ((wi + 1) % wpb)]!;
  const lo = off === 0 ? w0 : ((w0 >>> off) | (w1 << (32 - off))) >>> 0;
  return lo & ((1 << L) - 1);
}

/** Host decode of a whole packed [N, C] tensor to f32 (reference only). */
export function unpackDecodeHost(
  words: Uint32Array, scales: Float32Array, N: number, C: number, k: number, T: number, lut: Float32Array,
): Float32Array {
  const wpb = wordsPerBlock(T, k);
  const L = Math.log2(lut.length);
  const out = new Float32Array(N * C);
  const blocksPerRow = C / T;
  for (let n = 0; n < N; n++) {
    for (let b = 0; b < blocksPerRow; b++) {
      const bw = (n * blocksPerRow + b) * wpb;
      for (let t = 0; t < T; t++) out[n * C + b * T + t] = lut[stateAt(words, bw, wpb, t, T, k, L)]! * scales[n]!;
    }
  }
  return out;
}
