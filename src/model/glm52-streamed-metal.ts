...
function validateCall(
  ...
  if (
    ...
    throw new Error(
      `GLM Metal streamed expert decode requires input [1,${hidden}]`,
    );
  }
...
const GATE_UP_SOURCE = String.raw`
  ...
  for (uint word = lane; word < (uint)(H / 8); word += 32u) {
    ...
    for (uint place = 0; place < 8u; ++place) {
      const float xv = float(x[k0 + place]);
      const float gateWeight =
        (float((gatePacked >> (place * 4u)) & 0xFu) - 8.0f) * gateScale;
      const float upWeight =
        (float((upPacked >> (place * 4u)) & 0xFu) - 8.0f) * upScale;
      gateAcc = metal::fma(gateWeight, xv, gateAcc);
      upAcc = metal::fma(upWeight, xv, upAcc);
    }
  }

  const float gate = metal::simd_sum(gateAcc);
  const float up = metal::simd_sum(upAcc);
  if (lane == 0u) {
...
const DOWN_SOURCE = String.raw`
  ...
  for (uint word = lane; word < (uint)(I / 8); word += 32u) {
    ...
    const uint k0 = word * 8u;
    for (uint place = 0; place < 8u; ++place) {
      const float xv = float(mid[k0 + place]);
      const float weight =
        (float((packed >> (place * 4u)) & 0xFu) - 8.0f) * scale;
      acc = metal::fma(weight, xv, acc);
    }
  }
  const float value = metal::simd_sum(acc);
  if (lane == 0u) out[row] = T(value);
`;

/**
 * Bounded M=1 routed-SwiGLU candidate for the GLM-5.2 production decode
 * geometry. It deliberately uses two dispatches (gate+up+SiLU, then down);
 * batching/prefill stays on the stock MLX correctness path.
 */
export class Glm52CanonicalQ4MetalExecutor {
  ...
  execute(
    ...
    const [mid] = this.#gateUp.apply([input, slot], {
      outputs: [{
        shape: [1, layout.intermediateSize],
        dtype: input.dtype,
      }],
      grid: [
        THREADS,
    ...
    try {
      const [output] = this.#down.apply([mid, slot], {
        outputs: [{ shape: [1, layout.hiddenSize], dtype: input.dtype }],
        grid: [
          THREADS,
