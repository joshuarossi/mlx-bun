...
export function modelPromptBuilder(context: ServingContext): ModelServingBinding["buildPrompt"] {
  return context.serving?.buildPrompt ?? ((body, tools, ownership, prep, nativeWork, objects) =>
    buildModelPrompt(context as ServerContext, prep, body, tools, ownership, nativeWork, objects));
}
...
export function modelServingBinding(context: ServingContext): ModelServingBinding {
  ...
  return {
    ...
    buildPrompt: (body, tools, ownership, prep, nativeWork, objects) => buildModelPrompt(ctx, prep, body, tools, ownership, nativeWork, objects),
