// OpenAI-compatible speech-to-text routes:
//   POST /v1/audio/transcriptions   (task transcribe)
//   POST /v1/audio/translations     (task translate → English)
...
import { formatTranscription, TRANSCRIPTION_FORMATS, toVerboseJson, type TranscriptionFormat } from "../audio/whisper-format";
import type { WhisperSegment } from "../audio/whisper-transcribe";
import { TranscriptionError, TranscriptionService, type TranscriptionOutcome, type TranscriptionParams } from "./transcription-service";
...
/** Everything the route needs from the host: a service (or a reason there is none). */
export interface AudioRouteHost {
  service(): Promise<TranscriptionService | null>;
...
export async function handleAudioRoute(url: URL, request: Request, host: AudioRouteHost): Promise<Response | null> {
  const route = matchAudioRoute(request.method, url.pathname);
  if (!route) return null;
  const service = await host.service();
  if (!service)
    return errorJson(
      host.unavailableMessage ??
  ...
  if (route.kind === "session-audio" || route.kind === "session-finish" || route.kind === "session-delete") {
    ...
    if (route.kind === "session-audio") {
      ...
      try {
        ...
        } else pcm = await (service.constructor as typeof TranscriptionService).decodeAudio(bytes);
      } catch (e) {
        return transcriptionFailure(e);
      }
      try {
    ...
    try {
      ...
      sessionFormats.delete(route.id);
      return transcriptionResponse(outcome, sessionFormats.get(route.id) ?? "json", session.params.task ?? "transcribe");
    } catch (e) {
      sessionFormats.delete(route.id);
      return transcriptionFailure(e);
    }
