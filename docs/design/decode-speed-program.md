...
| `MLX_BUN_EARLY_FIRST_TOKEN` | Streaming/cancellation/overlap and cached/SSD measurements exist. Caller-owned early-return alignment preserves the ordinary M=1 cache boundary in native M1 and M4 packed/RTN4 checks. | Finish the monitored serving comparison; no gain for raw prompts. |
