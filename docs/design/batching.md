...
| Grammar and fill | Shared grammar proposals use grouped verification. Shared strict fill composes prefill, rows, sampling and committed append; native and HTTP/cache/timing gates pass | Shared echo uses grouped verification and row rollback; Qwen MTP consumes verified echo through the provider interface. Asserted fill with speculation, other providers and other named combinations remain. Keep grammar proposals opt-in after mixed timing results |
...
usage with faster concurrent completion; different batch geometries change some
responses. See [Qwen media evidence](../reference/benchmarks.md#shared-qwen-image-and-video-execution-2026-09-13)
for the source identity checks, probe correction and latency costs.
...
The affine same-B oracle now covers full-attention and rotating mixed-precision
rows, including unequal lengths, window crossing, reordering and retirement.
Equal-length rotating storage uses the pinned OptiQ implementation. Padded
rotating cases compose it with stock mlx-lm batch positions and masks; they do
not substitute solo-row logits for the batch oracle. The test-only reference
and its limits are described in [benchmarks.md](../reference/benchmarks.md#same-batch-rotating-affine-oracle).
Existing delayed-conversion, provider and lifecycle tests remain separate evidence.
