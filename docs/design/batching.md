...
| Grammar and fill | Shared grammar proposals use grouped verification. Shared strict fill composes prefill, rows, sampling and committed append; native and HTTP/cache/timing gates pass | Shared echo uses grouped verification and row rollback; fill combined with speculation and other named combinations remains. Keep grammar proposals opt-in after mixed timing results |
