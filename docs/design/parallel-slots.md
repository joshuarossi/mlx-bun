...
mlx-lm B=N oracle golden (`goldens/batched-golden-*.json`, machine-layered via
`tests/goldens.ts` — per-GPU overrides in `goldens/<machine>/` — generated by
`scripts/oracle/gen-batched-golden.py` in the oracle venv).
...
(would deviate). Oracle generator: `scripts/oracle/gen-batched-golden.py` (drives
...
join/leave mid-stream). `scripts/oracle/gen-batched-dynamic-golden.py` drives mlx-lm's
...
- Throughput ceiling: the B where we cross bandwidth→compute bound, and
  aggregate tok/s vs solo — measured via `scripts/bench-serve.ts all` (preflight-gated;
