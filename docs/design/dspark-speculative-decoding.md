...
## 27B program (2026-08-19): the TQ quant unlocks the real target

The turboquant-weights campaign shipped rot-sens48 Qwen3.8-27B at 17 GB
(docs/design/turboquant-weights.md) — and it changes both DSpark blockers
this doc recorded:

- **Target speed:** measured ~11.8 tok/s decode. This is the slow-target
  regime where the doc's own analysis says τ≈3 nets 2–3× (≈25–35 tok/s) —
  "the ideal target and the real motivation," now actually runnable.
- **Training feasibility:** "27B memory-infeasible to train on 24 GB" (open
  items below) assumed the bf16 trunk (54 GB). With the quantized trunk
  FROZEN (inference-only: hidden taps + teacher logits) and only the small
  drafter (~5 layers) holding grads/optimizer state in bf16, 27B-target
  drafter training fits a 32 GB machine. The premise is dead; the line in
  the gated item is superseded by this section.
- **Rollback risk retired:** the stock Qwen-MTP spec path already serves
  this model losslessly (68% per-token acceptance), so verify/rollback
  across the 48 DeltaNet recurrent-state layers is proven engine behavior,
  not new work.

Two tracks, cheap-first:
...
- **Track A — confidence-scheduled verification on the stock MTP head (no
  ...
  Gate: paired A/B decode tok/s vs fixed-γ MTP on the same prompts;
  lossless by construction (verify unchanged).
- **Track B — true DSpark drafter for the 27B (multi-token single-pass).**
  The faithful v2 (KV injection, Markov head) retargeted: `hiddenTap` added
  to `qwen3_5.ts` forwardLayers (gemma4 has it; parity-safe no-op when
  off), tap-layer selection for the 64-layer trunk (bias toward the 16
  full-attention layers + post-finalNorm), data generated by TEACHER-FORCED
  passes over corpus text with the frozen quantized trunk (every position
  of a forward yields a sample — data scale is forward-passes over tokens,
  not generate calls). Open idea: distill from / initialize against the
  shipped MTP head to shortcut the acceptance floor. Gate: generalizing
  per-position acceptance must beat the MTP head's 0.68 or Track B loses to
  Track A + MTP and gets dropped.

Sequencing: after the benchmark campaign closes (GPU is owned by the
certification suite). Track A first — it's measurable in a day and its
calibration data collection is also Track B's baseline instrumentation.
Adapters/DSpark drafts trained on the TQ artifact are married to its
rotation basis (fine for our bundle; note on the card if published).
