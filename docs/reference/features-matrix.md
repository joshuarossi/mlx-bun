...
| Qwen3.8-27B (same qwen3_5 graph) | ✅ L1 (no L2 oracle) | ✅ (SSM path) | thinking depths + preserve_thinking; native MTP head mounts via `--draft-model` (DeltaNet spec rollback = snapshot/replay, lossless-gated; quiet-box A/B: 0.82× — correct but slower, opt-in); **vision (images) SERVES** — bf16 tower sidecar + mRoPE + embed splice, oracle = mlx-vlm on pinned mlx (preprocessor/tower/positions bit-exact, e2e token-exact on 2/3 fixtures, serve smoke green; tests/qwen38-vision.test.ts); video FRAMES pipeline gated (T-aware resize, temporal pairs, per-frame-group attention) — video FILE decode awaits the AVFoundation sidecar's native-pack productization (probe green: lab/spikes/qwen38-video-sidecar) |
