...
| Qwen3.8-27B (same qwen3_5 graph) | ✅ L1 (no L2 oracle) | ✅ (SSM path) | thinking depths + preserve_thinking; native MTP head mounts via `--draft-model` (DeltaNet spec rollback = snapshot/replay, gate-proven lossless on the 0.8B hybrid; 27B pairing A/B is the gated tests/qwen38-mtp.test.ts); vision/video in bring-up |
