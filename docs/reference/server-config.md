...
| `MLX_BUN_TRELLIS_VARIANT` | — | `6` | Trellis matvec decode variant (Q2b kernels; design doc "Q2b measured"): `6` = code computed inline × reciprocal, weight served as f32 code×scale (fastest measured); `1` = the same with a residual step and bf16 rounding — bit-identical to the fake-quant artifact of record, ~40% slower; `0`/`2`/`3`/`4`/`5` are bench-only variants (precise divide, threadgroup LUT, device LUT, no-decode timing floor, unrounded-refined). The prefill/expand path always rounds to the activation dtype. |
