...
Encoder reuse is independent of batch/method selection. Experimental Gemma
media KV reuse is controlled by `MLX_BUN_MEDIA_PREFIX_CACHE` below. Cache counters
are documented in [server-api.md](server-api.md).
...
| `--whisper-model` | path \| query | first downloaded `whisper` checkpoint, resolved on the first `/v1/audio/*` request | both | Whisper checkpoint served beside the chat model for [audio transcription](server-api.md#post-v1audiotranscriptions--post-v1audiotranslations-speech-to-text). Serving a Whisper checkpoint as the MAIN model (`serve mlx-community/whisper-large-v3-turbo`) starts a **transcription-only server** (audio routes, `/v1/models`, `/health`, `/stats`; no chat model, no prompt cache). |
| `--whisper-idle-unload` | seconds | `0` | both | How long the Whisper weights (and MLX's allocator cache) stay after a take. Default `0`: released right after every take, so the chat model has the memory back before it prefills the dictated prompt; the process drops by ≈1.4 GB and the next take pages them back in from the OS file cache (a cold 11 s take is ≈520 ms vs 365 warm on the M1 Max, `TranscriptionService`). Reported per response in `mlx_bun.timings.load_ms`. |
| `--whisper-resident` | flag | off | both | Never dispose the Whisper weights (sotto's always-loaded behaviour). |
| `--preload` | flag | off | both | Transcription-only server: load the Whisper weights before listening instead of on the first request. |
...
| `MLX_BUN_MEDIA_PREFIX_CACHE` | (env only) | on (`=0` disables) | Reuse prepared Gemma image/audio or Qwen image/video conversation KV in the shared executor. Media contents, encoder identity and preceding tokens scope the ordinary RAM/SSD cache. Generated follow-ups reuse state; rewinds before a delayed-precision boundary recompute. Qwen recurrent layers require an exact retained prefix. Explicit serial retains encoder-only reuse. |
...
| `MLX_BUN_FILL` | — | **off** (`=strict` \| `=echo`) | **Token fast-forwarding.** Strict mode appends tool-call spans determined by the request's schema and chat template, then resumes sampling. Request-local parsing excludes ambiguous literals and incomplete delimiters; closed, single-required-property schemas qualify for key/call-close assertions. Echo mode additionally proposes text copied from the session and verifies it with the request's sampler. Shared execution supports seeded requests and the qualified bf16/affine/TurboQuant row layouts. Homogeneous echo cohorts use the existing grouped target verifier; mixed proposed/unproposed rows retain one-position sampling. Rejected suffixes are rolled back before reusable state is published. Wider verification can change floating-point rounding relative to sequential decode, so sampled text need not be identical. Explicit serial fill still excludes fixed seeds and uses its existing model/cache eligibility. Fill is disabled for logprobs, structured output, media, a mounted draft model and shared paging. Strict appends bypass sampling and therefore change behavior policy at nonzero temperature. Both modes remain opt-in; see the shared-fill section below and `usage.fill`. |
...
| `MLX_BUN_QWEN38_TQ_GRAPH` | on (`=0` disables) | The purpose-built graph for the published Qwen3.8-27B packed-Trellis quant (`src/model/qwen38-27b-trellis-tq.ts`). Selected only when the artifact's graph fingerprint (architecture plus the complete per-tensor quantization table) matches one the file was built for; every other artifact keeps the generic Qwen3.5 model. The graph owns the layer sequence and the MLP strategy per layer, fixed at load from the quant map; batch rows, window width and KV scheme stay runtime dimensions. With its arithmetic levers off it is byte-identical to the generic model through the batched lane. `=0` is the A/B control. Startup logs `[graph] qwen3.8-27b-trellis-tq <fingerprint> ...`. |
| `MLX_BUN_QWEN38_TQ_MIXED_GATEUP` | `split` (default), `fused`, `0` | Custom graph only. Layers whose gate and up projections were allocated different Trellis bit widths (q4b: 10 of 64) cannot use the same-width fused gate/up kernel and would run two projections plus a separate SwiGLU. One kernel templated on both widths restores the fusion: 1.23 ms per width-3 verify forward on the real tensors. `split` gives the kernel MLX's compiled SwiGLU arithmetic in bf16, bit-identical to the two-projection path it replaces (0 differing of 6,963,200 outputs across the 10 layers, M=1..4), so outputs do not change. `fused` (or the earlier `1`) uses the float32 sigmoid of the same-width fused kernel instead; last bits differ and sampled trajectories re-roll. `0`/`off` keeps the two projections: the A/B control. |
| `MLX_BUN_QWEN38_TQ_FOLD_QK_SCALE` | on (`=0` disables) | Custom graph only. DeltaNet computes `inv_scale² · rms_norm(q)` and `inv_scale · rms_norm(k)` as a weightless norm plus a scalar multiply. MLX's `rms_norm` kernel writes `w * T(x * inv)`, so the bf16 scalar rides in as a filled weight vector: the same product in the same type, one kernel instead of two, 96 fewer kernels per 27B forward. Bit-identical (`tests/unit/gdn-qk-scale-fold.test.ts`); bf16 activations only. `=0` is the A/B control. |
| `MLX_BUN_SPEC_DRAFT_VOCAB` | path to `{"ids":[...]}`; `0`/`off` disables (default: `<draft companion>/draft_vocab.json` when that file exists) | Frequency-ranked draft vocabulary (FR-Spec): the native MTP drafter projects onto the listed rows of the target's vocabulary head instead of all of them. The target still verifies and samples over the full vocabulary, so the output distribution is unchanged; an unlisted token cannot be drafted. Ids are kept ascending so the draft's categorical draw (MLX inverse CDF, one uniform from the step key) stays coupled to the target's. Qwen3.8-27B, 248,320 rows: projection 3.02 ms per draft step, 0.93 ms at 65,536 rows. Build lists with `reports/qwen38-trellis-publication/build-draft-vocab.ts`. A list shipped beside the MTP companion as `draft_vocab.json` loads with no flag; the env path overrides it and `0`/`off` restores full-vocabulary drafting. The list is tokenizer-specific, not quant-specific. A token outside the list can never be drafted, so text far from the list's corpus (English prose, code, agent transcripts) drafts worse; disable it there. Frozen-64 on the q4b artifact: 61/64 held, 18.915 -> 20.240 weighted decode tok/s together with the device-first round. Batched lane. |
| `MLX_BUN_SPEC_DRAFT_TEMPERATURE` | number ≥ 0 (default unset = request temperature) | Samples drafts at their own temperature. Measured no gain on identical-output items at 0.3 and 0.45 on the Qwen3.8 MTP head; kept as a lever. |
| `MLX_BUN_SPEC_DRAFT_FILTER` | on (`=0` disables) | `=0` drops top-p/top-k/min-p/XTC from the DRAFT draw only (about a quarter of the per-step draft sampling cost). The target's sampling is untouched. |
| `MLX_BUN_SPEC_DEVICE_ROUND` | on (`=0` disables) | Device-first speculative round in the shared batched lane. The draft tokens stay on the GPU, the verify input ids are assembled there, the GPU starts on the draft chain, and the verify graph (about 2,500 nodes built from JavaScript) is built while it runs; drafts and window samples then come back in one read. Same ops and values as the host-first order, so outputs are byte-identical (6/6 on the screen cut). Closes the per-round GPU idle gap: Qwen3.8-27B q4b MTP2 round 123.8 → 119.1 ms, GPU active 97.5% → 99.6%. Used when the drafter offers device proposals (Qwen MTP rows) and every row samples statelessly (plain greedy, or temperature/top-p/top-k without penalties, grammar or logprob capture); other rounds and `MLX_BUN_SPEC_PHASE_TIMING` keep the host-first order. |
| `MLX_BUN_SAMPLER_WINDOW_FILTERS` | on (`=0` disables) | For a multi-row verify window with the plain filter chain (top-p, top-k, temperature), run the filters once over `[W, V]` instead of once per position. Bit-identical per row at the served vocabulary size (`tests/unit/sampler-window.test.ts`); each row is still drawn separately with its own step key. About 0.5 ms per width-3 round. |
| `MLX_BUN_SPEC_LAYER_PROFILE` | `=1` (default off) | Diagnostic, requires `MLX_BUN_SPEC_PHASE_TIMING=1`. Component wall time INSIDE the served verify forward (Trellis MLP, DeltaNet projections/conv/norms/recurrence/output, attention projections/cache append/SDPA/output), reported as `usage.speculation.phaseMs.layers`. Adds an evaluation barrier per op, so the verify total grows while set. |
| `MLX_BUN_SPEC_OP_INVENTORY` | `=1` (default off) | Diagnostic, requires `MLX_BUN_SPEC_PHASE_TIMING=1`. Graph nodes built per op name for the verify forward and the draft chain (`phaseMs.verifyOps`, `phaseMs.draftOps`). |
| `MLX_BUN_SPEC_PHASE_TIMING` | `=1` (default off) | Diagnostic only. Each shared speculative round evaluates the verify logits before sampling and records wall milliseconds per phase; `usage.speculation.phaseMs` then carries `{draft, verify, sample, commit, rounds}` summed over the request. The forced evaluation removes the draft/verify overlap production decode relies on, so phase sums are attribution, not production round time. Serial spec loop unaffected. |
| `MLX_BUN_SPEC_GREEDY_DRAFT` | `=1` (default off) | Drafters select by argmax instead of the request sampler, in both lanes. Verification is exact token match against the target's own per-step sample, so emitted outputs keep the target distribution; only acceptance and per-draft cost change (the grouped Qwen MTP rows then skip logprob normalization and top-p/top-k/categorical work with one fused normalized argmax per draft step). Two-model drafting under this flag is no longer mlx-lm `speculative_generate_step` parity. Opt-in pending a paired acceptance/throughput A/B ([speculative-decoding.md §2](../design/speculative-decoding.md)). |
| `MLX_BUN_MTP_ADAPTIVE_DEPTH_THRESHOLD` | token count (default unset = fixed depth) | Explicit serial speculative loop only. With a Qwen MTP source configured at depth three or more, rounds draft two tokens while the logical context is below the threshold and the configured depth above it. The shared batched lane, which serves every default request, does not read it: the recorded q4b run with a 2,048 threshold was byte-identical to fixed MTP3 (55/64, 17.66 tok/s against 61/64 at 18.91 for fixed MTP2), so it measured depth three, not the policy. Unmeasured as a policy; default off. |
...
| Echo injection (session self-copy spans, verified against the same forward's logits) | off | shared bf16/affine/TurboQuant; explicit serial eligibility remains separate | Lab (paired A/B on task success + wall clock before any default) | `MLX_BUN_FILL=echo`, `MLX_BUN_FILL_K`, `MLX_BUN_FILL_CANDIDATES`, `MLX_BUN_FILL_INDEX_MAX` |
...
**Remaining exclusions:** fill combined with media, speculation or paging, and unsupported provider/layout
