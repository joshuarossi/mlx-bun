...
| `MLX_BUN_MEDIA_PREFIX_CACHE` | (env only) | off (`=1` enables) | Reuse prepared Gemma image/audio conversation KV in the shared executor. Media contents, encoder identity and preceding tokens scope the ordinary RAM/SSD cache. Generated follow-ups reuse state; rewinds before a delayed-precision boundary recompute. Qwen media and explicit serial retain encoder-only reuse. |
