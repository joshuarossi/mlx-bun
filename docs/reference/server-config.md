...
| `--max-tokens` | n ∈ [1, 1e7] | GLM-5.2: `128`; otherwise `65536` chat / `512` raw completion | both | Completion cap when a request omits `max_tokens` (mlx_lm.server flag; its default there is 512, so `--max-tokens 512` reproduces it). For GLM-5.2 it is also reserved by the pre-open resource equation and must fit inside `--context-length`. |
