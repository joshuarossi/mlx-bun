...
import { generateLibraryApi, libraryPackages } from "../scripts/generate-library-api";
...
`);
    await generateLibraryApi(repository, destination);
    ...
    expect(assertLibraryCoverage(project, expected)).toBe(7);
    ...
    expect(html.some(page => page.includes("https://github.com/joshuarossi/mlx-bun/blob/refactor/monorepo/packages/example/implementation.ts#L"))).toBe(true);

    const missingModule = structuredClone(project);
    missingModule.children!.pop();
    expect(() => assertLibraryCoverage(missingModule, expected)).toThrow("incomplete API coverage");
    const missingSymbol = structuredClone(project);
    missingSymbol.children![0]!.children!.pop();
    expect(() => assertLibraryCoverage(missingSymbol, expected)).toThrow("incomplete API coverage");
    ...
    expect(() => assertLibraryCoverage(brokenAlias, expected)).toThrow("unresolved API re-export");
