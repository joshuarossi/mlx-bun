...
import { ambientContextLine, APP_AWARE_TOOL_NAMES, applyEditedArgs, applySetSampling,
...
import { mapEventToFrames } from "../src/chat/events";
import { APP_ROUTE_IDS, type AppUiContext, type ServerMessage } from "../src/chat/protocol";

const MEMORY_TOOLS = ["test_memory_read", "test_reference_search"];
...
describe("injectAdapter (before_provider_request hook body)", () => {
  ...
  it("stacking: a composite 'a+b' selection is preserved as one wire field", () => {
...
describe("mapEventToFrames", () => {
  ...
  it("does not expose serving lane selection in turn_end", () => {
    expect(mapEventToFrames(ev({ type: "turn_end", message: { responseId: "anything" } })))
...
describe("webChatToolAllowlist", () => {
  it("is exactly the two welcome tools plus the app-aware tools when memory is off", () => {
    ...
    // codingTools state.
    expect(webChatToolAllowlist([])).toEqual([
  ...
  it("includes every memory + reference tool when memory is enabled", () => {
    ...
    const allow = webChatToolAllowlist(MEMORY_TOOLS);
    expect(allow).toContain("read");
    expect(allow).toContain("web_search");
    for (const t of MEMORY_TOOLS) expect(allow).toContain(t);
  });

  it("always includes the app-aware assistant tools (read-only, never gated)", () => {
    for (const codingTools of [false, true]) {
      for (const memory of [false, true]) {
        const allow = webChatToolAllowlist(memory ? MEMORY_TOOLS : [], codingTools);
  ...
  it("never widens beyond read-only tools by default (no bash/edit/write/web_fetch)", () => {
    for (const t of ["bash", "edit", "write", "web_fetch", "weather", "grep", "find", "ls"]) {
      expect(webChatToolAllowlist(MEMORY_TOOLS)).not.toContain(t);
      expect(webChatToolAllowlist(MEMORY_TOOLS, false)).not.toContain(t);
    }
  });

  it("adds bash/edit/write/grep/find/ls ONLY when codingTools is explicitly true (plan §5.4/§6.5 opt-in toggle)", () => {
    const allow = webChatToolAllowlist(MEMORY_TOOLS, true);
