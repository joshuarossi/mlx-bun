...
import { getEmbedCounter, resetEmbedCounter } from "./support/tripwire";
import { createMemoryTools, MEMORY_TOOL_NAMES, REFERENCE_TOOL_NAMES } from "../../src/memory/tools";
import { fixtureWiki } from "./support/fixture-wiki";

// The read tools are bound to one vault root at construction, so they point at
// the fixture vault without writing the real ~/.mlx-bun/wiki.
const FIXTURE_VAULT = fixtureWiki();
afterAll(() => rmSync(FIXTURE_VAULT, { recursive: true, force: true }));

const tools = createMemoryTools(FIXTURE_VAULT);
...
describe("north-star traversal — FIND → READ → ANSWER over the fixture vault", () => {
  ...
  it("registers no infobox-field-query tool (the infobox is content, not a filter)", () => {
    ...
    // One factory registers the memory AND reference read tools here (main built them separately).
    expect(tools.map((t) => t.name).sort()).toEqual([...MEMORY_TOOL_NAMES, ...REFERENCE_TOOL_NAMES].sort());
  });
});
