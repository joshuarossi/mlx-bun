...
import { ChatRequest, TextCompletionRequest } from "../../src/server/chat-request";
import { ChatStage } from "../../src/server/chat-stage";
import { TextCompletionStage } from "../../src/server/text-completion-stage";
import { InferenceStage } from "../../src/server/inference-request";
import { RequestError } from "../../src/server/pipeline";
import { createRequestPrep } from "../../src/server/request-prep";
import { CompletionExecutor, type CompletionEngine } from "../../src/server/completion-executor";
import type { CompletionEvent } from "../../src/server/completion-sink";
import type { ModelContext } from "../../src/engine/model-host";
import { chatCompletionJson, chatCompletionStream } from "../../src/server/openai-wire";
...
class ScriptedEngine implements CompletionEngine {
  ...
  place(shape: Parameters<CompletionEngine["place"]>[0]) {
    return Object.freeze({ shape, mechanism: "continuous" as const, execution: { method: "autoregressive", mechanism: "continuous" as const, pagedKv: false, promptCache: true, checkpoint: true, fill: false, compiledDecode: false, grammarJump: false, reasons: [] } });
  }
  async run(
    _promptIds: number[],
...
function harness(overrides: { contextLimit?: number; defaultAdapter?: string;
  preparation?: import("../../src/engine/preparation").PreparationExecutor;
  buildPrompt?: import("../../src/server/prompt-contracts").ModelPromptBuilder } = {}) {
...
describe("ChatStage: ChatRequest → InferenceRequest", () => {
  ...
  test("an injected prompt builder's unsupported media error is a 400", async () => {
    const { chat } = harness({ buildPrompt: async () => { throw new RequestError(400, "model does not accept video input"); } });
...
describe("InferenceStage: admit, then run", () => {
  test("the whole pipe: result carries content, finish reason, usage, lane", async () => {
    ...
    expect(result).toMatchObject({
      content: "t1 t2", reasoning: "", toolCalls: [], lane: "batched",
    ...
    expect(chatCompletionJson(result, { id: "chatcmpl-x", created: 1, model: "fake-chat" })).toMatchObject({
      ...
      usage: { prompt_tokens: 3, completion_tokens: 2, total_tokens: 5, prompt_tokens_details: { cached_tokens: 1 }, lane: "batched" },
    });
  });
...
test("raw completion grammar preparation respects engine admission and abort cleanup", async () => {
  const abort = new AbortController();
  let released = 0, disposed = 0, nativeHeld = false;
  const grammar = { dispose() { disposed++; } };
  const ctx = {
    tokenizer: { encode: () => [1], idToToken: () => "t" },
    adapters: { resolveSpec: () => [] }, draft: null,
  } as unknown as ModelContext;
  const prep = {
    toOptions: () => ({ stopSequences: [] }),
    async compileGrammarForRequest() {
      expect(nativeHeld).toBe(true);
      abort.abort(new Error("disconnected"));
      return { controller: grammar, degradeHint: null };
    },
  } as unknown as ReturnType<typeof createRequestPrep>;
  const stage = new TextCompletionStage(ctx, prep, null, undefined, undefined, {
    async reserve(kind) { expect(kind).toBe("constraint"); return { dispose() { released++; } }; },
    async run(work) { nativeHeld = true; try { return await work(); } finally { nativeHeld = false; } },
  });
  await expect(stage.run(new TextCompletionRequest({ prompt: "hi", response_format: { type: "json_object" } }), undefined, abort.signal)).rejects.toThrow("disconnected");
  expect(released).toBe(1); expect(disposed).toBe(1); expect(nativeHeld).toBe(false);
});
