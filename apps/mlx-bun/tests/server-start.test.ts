...
test("the mounted app serves implemented routes, explicit migration gaps, and unknown paths separately", async () => {
  ...
  try {
    ...
    for (const path of ["/api/settings/hf-token", "/api/quantize/push", "/api/dataset/push", "/v1/memory/synthesize", "/v1/audio/transcriptions", "/admin/cache/flush", "/stats", "/api/hub/local", "/api/hub/search", "/api/hub/download",
