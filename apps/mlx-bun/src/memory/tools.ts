...
export async function isMemoryEnabled(root: string): Promise<boolean> {
  try {
    return (await vaultStatus(root)).exists;
  } catch {
    return false;
  }
}
...
export function createMemoryTools(root: string): ToolDefinition[] {
  ...
  async function memoryUnavailableMessage(): Promise<string | null> {
    ...
    return `Memory is not set up yet at ${st.root}. Use the memory panel to initialize the local Markdown wiki with the user’s agreement.`;
  }
    ...
    execute: async (_id, params) => {
      ...
      try {
        ...
        for (const other of stems) {
          ...
          try {
            const oc = (await readArticle(root, other)).content;
  ...
  const memorySearchTool = defineTool({
    ...
    execute: async (_id, params) => {
      ...
      try {
        const unavailable = await memoryUnavailableMessage();
        if (unavailable) return textResult(unavailable);
        const { summaries, hits } = await searchArticles(root, query, { limit, scope: "articles" });
  ...
  const referenceSearchTool = defineTool({
    ...
    execute: async (_id, params) => {
      ...
      try {
        const unavailable = await memoryUnavailableMessage();
        if (unavailable) return textResult(unavailable);
        const { summaries, hits } = await searchArticles(root, query, { limit, scope: "reference" });
  ...
  const referenceReadTool = defineTool({
    ...
    execute: async (_id, params) => {
      ...
      try {
        ...
        const { content } = await readArticle(root, id);
        return textResult(content);
      } catch (err) {
  ...
  const referenceListTool = defineTool({
    ...
    execute: async () => {
      try {
        const unavailable = await memoryUnavailableMessage();
        if (unavailable) return textResult(unavailable);
        const refs = await listReferenceDocs(root);
  ...
  return [
    memoryResolveTool, memoryCategoryTool, memoryReadTool, memorySectionTool,
    memoryLinksTool, memoryInfoboxTool, memoryListTool, memoryStatusTool, memorySearchTool,
    referenceSearchTool, referenceReadTool, referenceListTool,
  ];
}
 ...
 */
export async function memoryIndexHint(root: string): Promise<string> {
  let st: Awaited<ReturnType<typeof vaultStatus>>;
  try {
    st = await vaultStatus(root);
  } catch {
    return "";
  }
