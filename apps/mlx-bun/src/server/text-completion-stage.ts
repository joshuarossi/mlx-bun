...
export class TextCompletionStage {
  constructor(
    ...
    private readonly preparation?: import("../engine/preparation").PreparationExecutor,
  ) {}

  async run(request: TextCompletionRequest, requestId?: string, signal?: AbortSignal): Promise<InferenceRequest> {
    signal?.throwIfAborted();
    const body = request.params;
    const constrained = body.response_format != null || !!body.guided_grammar ||
      !!body.guided_regex || !!body.guided_choice?.length || body.structured_outputs != null;
    let reservation = constrained ? await this.preparation?.reserve?.("constraint", signal) : undefined;
    try {
      const build = async () => {
        signal?.throwIfAborted();
        const result = await this.runPrepared(request, requestId);
        if (signal?.aborted) { result.plan.ownership.dispose(); signal.throwIfAborted(); }
        return result;
      };
      const result = constrained && this.preparation ? await this.preparation.run(build, signal) : await build();
  ...
  private async runPrepared(request: TextCompletionRequest, requestId?: string): Promise<InferenceRequest> {
    const { ctx, prep } = this;
    const body = request.params;
