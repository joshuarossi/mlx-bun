...
import {
  ...
  FillSession,
  resolveFillMode,
} from "@mlx-bun/inference/generation/fill";
import { compileStrictFillRows, type StrictRowPlan } from "@mlx-bun/inference/generation/fill";
...
export function createRequestPrep(input: {
  ...
  const toOptions = (req: ChatRequestParams): GenerateOptions & { stopSequences: string[] } => {
    ...
    return {
      ...
      ...kvScheme,
      // Cache layout is supplied by the application; the execution binding
      // validates whether the selected method can consume it.
  ...
  ): Promise<{ controller: import("@mlx-bun/inference/sampling/grammar").GrammarController | null; degradeHint: string | null }> => {
    if (runtimeValue("MLX_BUN_GRAMMAR") === "0") {
      return {
        controller: null,
