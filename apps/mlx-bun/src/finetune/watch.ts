...
// (renderFrame) over parsed state, so it can be exercised against a synthetic
// stream with no TTY; the loop takes its terminal, file reads, and clock as
// explicit ports for the same reason.
...
/** The terminal the loop draws on. Key input and resize notification are
 *  optional: absent when stdin is not a TTY, as with a piped run. */
export interface WatchTerminal {
  write(text: string): void;
  size(): { columns: number; rows: number };
  /** Raw key input; returns a detach function restoring the previous mode. */
  keys?(onKey: (key: string) => void): () => void;
  onResize?(listener: () => void): () => void;
}

export interface WatchOptions {
  /** External stop (the CLI's SIGINT/SIGTERM); the loop restores the terminal and returns. */
  signal?: AbortSignal;
...
/** Tail `<adapterDir>/metrics.jsonl` on the alternate screen until `q`, ^C in
 *  raw mode, or the external signal; the terminal is restored on every exit
 *  path. Throws when the run directory has no metrics stream. */
export async function runWatch(adapterDir: string, options: WatchOptions = {}): Promise<void> {
  const path = `${adapterDir}/metrics.jsonl`;
  const exists = options.exists ?? existsSync, read = options.read ?? ((p: string) => readFileSync(p, "utf8"));
  if (!exists(path)) {
    throw new Error(`no metrics.jsonl in ${adapterDir} — is this an mlx-bun training run dir?\n`
      + `(the trainer writes ${path} as it runs; point train-watch at the --adapter dir)`);
  }
  const term = options.terminal ?? processTerminal();
  const sleep = options.sleep ?? sleepUntil;
  const quit = new AbortController();
  const stop = () => quit.abort();
  if (options.signal?.aborted) stop(); else options.signal?.addEventListener("abort", stop, { once: true });

  let lastRender = "";
  const draw = (st: RunState) => {
    const { columns, rows } = term.size();
    const frame = HOME + renderFrame(st, columns, rows).join("\n");
