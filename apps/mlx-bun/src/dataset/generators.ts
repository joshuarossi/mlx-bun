// The supported dataset generators, ported from optiq's dataset_templates.py
...
/** Preserve the importer's three attempts, with cancellable backoff. */
async function hfFetchJson(url: string, http: DatasetHttp): Promise<any> {
  const headers = hfAuthHeaders();
  let lastErr: unknown;
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      http.signal?.throwIfAborted();
      const res = await (http.fetch ?? fetch)(url, { headers, signal: http.signal });
      if (res.ok) return await res.json();
      // Keep response details on other failures; the catch below retries them too.
    ...
    if (attempt < 2) await delay(500 * (attempt + 1), undefined, { signal: http.signal });
  }
  throw lastErr instanceof Error ? lastErr : new Error(String(lastErr));
}

/** Resolve the default config for a dataset (first config of the split, or any). */
async function resolveDefaultConfig(hfId: string, split: string, http: DatasetHttp): Promise<string> {
  const url = `${HF_SPLITS_ENDPOINT}?dataset=${encodeURIComponent(hfId)}`;
  const data = await hfFetchJson(url, http);
...
export async function genHfDatasetImport(
  ...
  if (!config) {
    config = await resolveDefaultConfig(hfId, split, http);
  }
