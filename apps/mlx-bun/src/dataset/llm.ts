...
export interface DatasetHttp { signal?: AbortSignal; fetch?: typeof fetch }
 ...
 */
export function makeLlmClient(apiUrl: string, model = "local", http: DatasetHttp = {}): LlmClient {
  ...
  async function chatRaw(messages: ChatMessage[], opts: ChatOpts = {}): Promise<any> {
    ...
    const resp = await (http.fetch ?? fetch)(`${base}/v1/chat/completions`, {
      method: "POST",
      signal: http.signal,
      headers: {
