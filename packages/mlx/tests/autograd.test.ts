// Native autograd (ValueAndGrad / Vjp) finite-difference check. Weight-free;
// runs wherever the mlx native library is staged (native Mac CI), and fails
// at import like its neighbours when it is not.
//
// Exercises the encapsulated ValueAndGrad class on a LoRA-shaped loss
...
// that exactly len(argIdx) grads come back. Vjp (mlx_vjp), the segmented-
// backward backbone, must agree with ValueAndGrad for a scalar loss with
// cotangent 1.0 and seed a vector-output backward with its cotangent.

import { describe, expect, test } from "bun:test";
import { MlxArray } from "@mlx-bun/mlx/array";
import { ValueAndGrad, Vjp } from "@mlx-bun/mlx/autograd";
...
// Vjp (mlx_vjp) — the segmented-backward backbone (@mlx-bun/training/segmented).
