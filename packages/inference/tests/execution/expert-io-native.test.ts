...
describe("native expert I/O slabs", () => {
  ...
  it("scatter-loads disjoint regions from multiple shards into one slot", async () => {
    const store = new ExpertIOSlabStore(
      [file, secondFile],
      { slots: 1, slotBytes: 16 * 1024, workers: 2 },
    );
    try {
  ...
  it("runs advisory hints on a separate queue without publishing a slot", async () => {
    const store = new ExpertIOSlabStore(
      [file, secondFile],
      { slots: 1, slotBytes: 16 * 1024, workers: 1 },
    );
    try {
  ...
  it("loads positioned bytes into stable aligned slots and fences reuse", async () => {
    const store = new ExpertIOSlabStore(file, { slots: 2, slotBytes: 16 * 1024, workers: 2 });
    try {
  ...
  it("reports short reads and rejects stale generations", async () => {
    const store = new ExpertIOSlabStore(file, { slots: 1, slotBytes: 16 * 1024, workers: 1 });
    try {
  ...
  it("feeds one registered slab to stock MLX and custom Metal without staging", async () => {
    const store = new ExpertIOSlabStore(floatFile, { slots: 1, slotBytes: floatData.byteLength, workers: 1 });
