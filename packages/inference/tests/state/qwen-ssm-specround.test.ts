// DeltaNet prefix replay and recurrent-state ownership, using generated inputs.
import { describe, expect, test } from "bun:test";
import { MlxArray } from "@mlx-bun/mlx/array";
import { Dtype } from "@mlx-bun/mlx/ffi";
import { SSMCache } from "../../src/state/ssm";
import { gatedDeltaUpdate } from "../../src/kernels/delta/gated";
import { gatedDeltaState } from "../../src/kernels/delta/state";
import { createHash } from "node:crypto";
import * as ops from "@mlx-bun/mlx/ops";

const HK = 2, HV = 4, DK = 64, DV = 64, T = 3;
const values = (count: number, phase: number) => Array.from({ length: count }, (_, i) => Math.sin(i * 0.17 + phase) * 0.1);
const g = { HK, HV, DK, DV, A_log: values(HV, 1), dt_bias: values(HV, 2),
  prefill: { T, q: values(T * HK * DK, 3), k: values(T * HK * DK, 4), v: values(T * HV * DV, 5), a: values(T * HV, 6), b: values(T * HV, 7) } };
