...
import { splitKTrellisPrefill, splitKTrellisPrefillEligible } from "@mlx-bun/inference/kernels/trellis";
...
test("split-K trellis prefill matches stock expansion on other shapes and partition tails", () => {
  for (const bits of [2, 3, 4]) for (const [k, n, m] of [[1280, 256, 5], [2080, 512, 8], [4096, 768, 5]]) {
    ...
    const g = { k: bits, L: 12, T: 256, axis: 0 as const, rows: k!, cols: n!, inFeatures: k!, outFeatures: n! };
    ...
    const expanded = expandTrellis(codes, scales, g, Dtype.bfloat16, 13);
