...
//   - uploads use only the caller-supplied token
...
import {
  uploadFolder,
  createRepo,
} from "@mlx-bun/hub/upload";
import { createRepo as publicCreateRepo, uploadFolder as publicUploadFolder } from "@mlx-bun/hub";
...
function temporary(prefix: string) { const dir = mkdtempSync(join(tmpdir(), prefix)); directories.push(dir); return dir; }

function makeModelDir(): string {
  const dir = temporary("mlx-bun-hfpush-");
...
describe("uploadFolder (mock Hub)", () => {
  ...
  test("dataset repo uses the datasets/ URL prefix throughout", async () => {
    const dir = temporary("mlx-bun-hfds-");
    ...
    try {
      ...
      expect(cap.paths).toContain("/api/datasets/me/my-dataset/commit/main");
      expect(cap.paths).toContain("/datasets/me/my-dataset.git/info/lfs/objects/batch");
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  });
  ...
  test("surfaces a clear error on 401", async () => {
    const dir = temporary("mlx-bun-hf401-");
  ...
  test("null authentication stays anonymous throughout the upload and public exports agree", async () => {
    expect(publicCreateRepo).toBe(createRepo);
    expect(publicUploadFolder).toBe(uploadFolder);
    await uploadFolder(makeModelDir(), "me/anonymous", { token: null, baseUrl: base });
    expect([cap.createAuth, cap.preuploadAuth, cap.lfsBatchAuth, cap.commitAuth]).toEqual([null, null, null, null]);
  });

  test("missing explicit tokens reject before sending any request", async () => {
    // @ts-expect-error Callers must explicitly choose a token or null.
  ...
  test("file walking skips VCS/cache directories and symlinks as in main", async () => {
    ...
    for (const name of [".git", ".cache"]) {
      mkdirSync(join(dir, name)); writeFileSync(join(dir, name, "private.txt"), "excluded");
    }
    symlinkSync(join(dir, "config.json"), join(dir, "config-link.json"));
    await uploadFolder(dir, "me/filtered", { token: null, baseUrl: base });
