...
const M=512, H=2560, V=262144, GS=64, BITS=8;       // e4b head
...
const BODY = String.raw`
  ...
  threadgroup float Xs[BM*BKp]; threadgroup float Ws[BN*BKp];
  ...
  const uint3 tid=threadgroup_position_in_grid; const uint sg=simdgroup_index_in_threadgroup, sl=thread_index_in_simdgroup;
  const int yr=tid.y*BM, yc=tid.x*BN;
  lx_t loader_x(x+(int64_t)yr*Kk, Kk, Xs, sg, sl);
  lw_t loader_w((const device uint8_t*)w+yc*K_w, scales+yc*K_g, biases+yc*K_g, Kk, Ws, sg, sl);
  mma_t mma_op(sg, sl);
...
time("STEEL PORT (our kernel)", steel);
time("MLX quantizedMatmul", ref);
