...
const ALLOW: AllowEntry[] = [
  ...
  // LoRA test skips cleanly when the files are absent. Plan B2 is CLOSED.
  {
    ...
    maxBytes: 1_200_000,
    why: "Text JSON golden (bf16-quantized reference kernel output). Model-free CI load-bearer (tests/unit/qwen-delta.test.ts, no model load). Regenerated by scripts/oracle/gen-qwen-delta-golden.py.",
  },
  {
    glob: "tests/fixtures/**/*.bin",
    maxBytes: 16_384,
    why: "Model-free universal-rope bit-exact oracle fixtures (4–8 KB each). CI load-bearer; tests/parity/universal-rope.test.ts skips cleanly if absent. Regenerated by scripts/oracle/gen-universal-rope-fixtures.py.",
  },
  {
...
// ---------------------------------------------------------------------------
// 10. tests: the directory IS the gate. tests/{unit,serve,using} run in CI on
//     a machine with no weights and no oracle venv, so nothing there may
//     depend on a model snapshot or an MLX_BUN_TEST_* opt-in. Such tests live
//     in tests/parity or tests/research. (Fixture-presence skips are fine.)
// ---------------------------------------------------------------------------

function checkTestTiers(): string[] {
  const fails: string[] = [];
  const banned = /MLX_BUN_TEST_[A-Z0-9_]+|from "\.\.\/support\/paths"|hfSnapshot\(|SNAPSHOT_[A-Z0-9]+\b/;
  for (const f of listFiles().filter((f) => /^tests\/(unit|serve|using)\/.*\.test\.ts$/.test(f))) {
    const text = readFileSync(`${ROOT}/${f}`, "utf8");
    const m = text.match(banned);
    if (m) fails.push(`  FAIL  ${f} — CI-tier test depends on weights/opt-in (${m[0]}). Move it to tests/parity/ or tests/research/.`);
  }
  return fails;
}
