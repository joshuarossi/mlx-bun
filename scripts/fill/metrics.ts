...
export function fillVerdict(
  ...
  const verdict = n === 0
    ...
    : pass
      ...
      : !agreementHolds
        ? `FAIL — task agreement ${agreementDelta < 0 ? "dropped" : "not shown to hold (more paired turns needed)"}: ` +
          `${pct(a.taskAgreement)} → ${pct(b.taskAgreement)} (delta ${(agreementDelta * 100).toFixed(1)} pts, ` +
