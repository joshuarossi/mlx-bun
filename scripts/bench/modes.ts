...
//   - scripts/bench-matrix.ts features (feature composition: grammar/spec)
//   - scripts/bench-serving-load.ts   (client-only, stack-vs-stack)
//   - bun scripts/bench-serve.ts all                  (preflight-gated h2h, quotable rows)
//
//   bun scripts/bench-matrix.ts modes [--model <snapshot-dir>] [--draft <dir>]
...
if (!MODEL || !existsSync(`${MODEL}/config.json`)) {
  console.error("usage: bun scripts/bench-matrix.ts modes [--model <snapshot-dir>] [--draft <dir>] [--repeats 3] [--maxtok 128] [--quick] [--cells all]");
