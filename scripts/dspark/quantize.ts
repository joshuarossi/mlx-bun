...
// (scripts/dspark.ts ab) — run it before adopting any rung.

import { quantizeDrafterDir } from "../../src/spec/dspark/quantize-drafter";

function usage(): never {
  console.error(
    "usage: bun scripts/dspark.ts quantize <drafter-dir> [--out <dir>] [--bits 4|8] [--group-size 32|64]",
  );
  process.exit(2);
}
...
console.log(`  bun scripts/dspark.ts ab --target <target-model> --drafter-a ${src} --drafter-b ${outDir}`);
